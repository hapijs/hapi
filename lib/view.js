...
// Van's Debug functions
var Log = require("multitool").Log;
var l = new Log();
var debug = l.debug;
var trace = l.trace;
...
exports = module.exports = internals.View = function (options) {
    ...
    trace('new View()', arguments);
    this.options = Utils.merge(Utils.clone(this._defaultOptions), options);
    debug('View options', JSON.stringify(this.options, null, 2));
    
    this.engine = require(this.options.engine.name || 'handlebars');
    
    if (this.options.partials) {
        // TODO: convert to asserts
        if (!this.engine.hasOwnProperty('registerPartial')) {
            throw this.options.engine.name + " partials are not supported at this time";
        }
        
        if (!this.options.partials.path) {
            throw "options.partials.path must be defined";
        }
...
internals.View.prototype.initPartials = function () {

    // this.walk(this.options.partials.path, {}, 
    var opts = {
        include: ['html']
    };
    
    // this.walk(this.options.partials.path, opts, function (err, files) {
    ...
    //         // this.registerPartial(files[file]);
...
internals.View.prototype.isAbsPath = function (path) {
    ...
    trace('View.isAbsPath', arguments);
...
internals.View.prototype.isInsecureAccessAttempt = function (path) {
    ...
    trace('View.isInsecureAccessAttempt', arguments);
...
internals.View.prototype.getExtension = function (filename) {
    ...
    trace('View.getExtension', arguments);
    filename = filename || "";
...
internals.View.prototype.getPath = function (path) {
    ...
    trace('this.options.path', this.options.path);
    trace("this.getExtension(path)", this.getExtension(path));
