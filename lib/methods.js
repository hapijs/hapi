...
internals.Methods.prototype._add = function (name, fn, options, env) {
    ...
    var cache = this.server._provisionCache(settings.cache, 'method', name, settings.cache.segment);
    ...
    var method = function (/* arguments, methodNext */) {
        ...
        if (key === null || typeof key !== 'string') {                             // Value can be ''
            self.server.log(['hapi', 'method', 'key', 'error'], { name: name, args: args, key: key });
            key = null;
        }
