// Load modules

var Boom = require('boom');
var Catbox = require('catbox');
var Hoek = require('hoek');
var Schema = require('./schema');


// Declare internals

var internals = {};


exports = module.exports = internals.Methods = function (server) {

    this.server = server;
    this.methods = {};
    this._normalized = {};
};


internals.Methods.prototype.add = function (name, method, options, env) {

    if (typeof name !== 'object') {
        return this._add(name, method, options, env);
    }

    // {} or [{}, {}]

    var items = [].concat(name);
    for (var i = 0, il = items.length; i < il; ++i) {
        var item = items[i];
        this._add(item.name, item.method, item.options, env);
    }
};


exports.methodNameRx = /^[a-zA-Z]\w*(?:\.[a-zA-Z]\w*)*$/;


internals.Methods.prototype._add = function (name, method, options, env) {

    var self = this;

    Hoek.assert(typeof method === 'function', 'method must be a function');
    Hoek.assert(typeof name === 'string', 'name must be a string');
    Hoek.assert(name.match(exports.methodNameRx), 'Invalid name:', name);
    Hoek.assert(!Hoek.reach(this.methods, name, { functions: false }), 'Server method function name already exists');

    options = options || {};
    Schema.assert('method', options, name);

    var settings = Hoek.cloneWithShallow(options, ['bind']);
    settings.generateKey = settings.generateKey || internals.generateKey;
    var bind = settings.bind || env.bind || null;

    // Normalize methods

    var normalized = method;
    if (settings.callback === false) {          // Defaults to true
        normalized = function () {

            var methodNext = arguments[arguments.length - 1];

            var timer = new Hoek.Timer();
            var result = null;
            var error = null;

            try {
                result = method.apply(bind, Array.prototype.slice.call(arguments, 0, arguments.length - 1));
            }
            catch (err) {
                error = err;
            }

            if (result instanceof Error) {
                error = result;
                result = null;
            }

            var finalize = function (error, result) {

                if (settings.cache) {
                    return methodNext(error, result);
                }

                return methodNext(error, result, null, { msec: timer.elapsed(), error: error });
            };

            if (error ||
                typeof result !== 'object' ||
                typeof result.then !== 'function') {

                return finalize(error, result);
            }

            // Promise object

            var onFulfilled = function (result) {

                return finalize(null, result);
            };

            var onRejected = function (err) {

                return finalize(err);
            };

            result.then(onFulfilled, onRejected).catch(onRejected);
        };
    }

    // Not cached

    if (!settings.cache) {

        // Normalize method

        if (settings.callback !== false) {          // Defaults to true
            normalized = function (/* arguments, methodNext */) {

                var args = arguments;
                var methodNext = args[args.length - 1];

                var timer = new Hoek.Timer();
                args[args.length - 1] = function (err, result) {

                    methodNext(err, result, null, { msec: timer.elapsed(), error: err });
                };

                method.apply(bind, args);
            };
        }

        return this._assign(name, bind ? method.bind(bind) : method, normalized);
    }

    // Cached

    Hoek.assert(!settings.cache.generateFunc, 'Cannot set generateFunc with method caching');

    settings.cache.generateFunc = function (id, next) {

        id.args[id.args.length - 1] = next;                 // function (err, result, ttl)
        normalized.apply(bind, id.args);
    };

    var cache = this.server.cache(settings.cache, '#' + name);

    var func = function (/* arguments, methodNext */) {

        var args = arguments;
        var methodNext = args[args.length - 1];

        var key = settings.generateKey.apply(bind, args);
        if (key === null ||                                 // Value can be ''
            typeof key !== 'string') {                      // When using custom generateKey

            return Hoek.nextTick(methodNext)(Boom.badImplementation('Invalid method key when invoking: ' + name, { name: name, args: Array.prototype.slice.call(arguments, 0, arguments.length - 1) }));
        }

        cache.get({ id: key, args: args }, methodNext);
    };

    func.cache = {
        drop: function (/* arguments, callback */) {

            var dropCallback = arguments[arguments.length - 1];

            var key = settings.generateKey.apply(null, arguments);
            if (key === null) {                             // Value can be ''
                return Hoek.nextTick(dropCallback)(Boom.badImplementation('Invalid method key'));
            }

            return cache.drop(key, dropCallback);
        }
    };

    this._assign(name, func, normalized);
};


internals.Methods.prototype._assign = function (name, method, normalized) {

    var path = name.split('.');
    var ref = this.methods;
    for (var i = 0, il = path.length; i < il; ++i) {
        if (!ref[path[i]]) {
            ref[path[i]] = (i + 1 === il ? method : {});
        }

        ref = ref[path[i]];
    }

    this._normalized[name] = normalized;
};


internals.generateKey = function () {

    var key = 'h';                                                   // 'h' for helper
    for (var i = 0, il = arguments.length - 1; i < il; ++i) {        // 'arguments.length - 1' to skip 'next'
        var arg = arguments[i];
        if (typeof arg !== 'string' &&
            typeof arg !== 'number' &&
            typeof arg !== 'boolean') {

            return null;
        }

        key += ':' + encodeURIComponent(arg.toString());
    }

    return key;
};
