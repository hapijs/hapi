...
exports.parseCookies = function (request, next) {
    ...
    var verify = cookies.replace(/\s*([^\x00-\x20\(\)<>@\,;\:\\"\/\[\]\?\=\{\}\x7F]+)\s*=\s*(?:(?:"([^\x00-\x20\"\,\;\\\x7F]*)")|([^\x00-\x20\"\,\;\\\x7F]*))(?:(?:;|(?:\s*\,)\s*)|$)/g, function ($0, $1, $2, $3) {
        ...
        if (request.server._states[name] &&
            request.server._states[name].encoding &&
            request.server._states[name].encoding !== 'none') {

            // Encodings: 'base64json', 'base64', 'form', 'none'

            try {
                if (request.server._states[name].encoding === 'base64') {
                    value = (new Buffer(value, 'base64')).toString('binary');
                }
                else if (request.server._states[name].encoding === 'base64json') {
                    var decoded = (new Buffer(value, 'base64')).toString('binary');
                    value = JSON.parse(decoded);
                }
                else if (request.server._states[name].encoding === 'form') {
                    value = Querystring.parse(value);
                }
            }
            catch (e) {

                error = { name: name, value: value, settings: request.server._states[name] };
        ...
        if (request.cookies[name]) {
            if (request.cookies[name] instanceof Array === false) {
                request.cookies[name] = [request.cookies[name]];
            }

            request.cookies[name].push(value);
        }
        else {
    ...
    if (error ||
        ...
        if (request.server.settings.state.cookies.failAction === 'log') {
            request.log(['cookies', 'error'], error || cookies);
        }
    }
...
exports.generateSetCookieHeader = function (cookies, states) {
    ...
    for (var i = 0, il = cookies.length; i < il; ++i) {
        ...
        if (states[cookie.name]) {
            options = Utils.applyToDefaults(states[cookie.name], options);
        }
        ...
        if (options.ttl) {
            var expires = new Date(Date.now() + options.ttl);
