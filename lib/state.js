...
internals.parseRx = {
    ...
    strict: /\s*([^\x00-\x20\(\)<>@\,;\:\\"\/\[\]\?\=\{\}\x7F]+)\s*=\s*(?:(?:"([^\x00-\x20\"\,\;\\\x7F]*)")|([^\x00-\x20\"\,\;\\\x7F]*))(?:(?:;|(?:\s*\,)\s*)|$)/g,
    loose:  /\s*([^=\s]+)\s*=\s*(?:(?:"([^\"]*)")|([^\;]*))(?:(?:;|(?:\s*\,)\s*)|$)/g
};
...
exports.parseCookies = function (request, next) {
    ...
    var header = function (cookies) {
        ...
        var state = {};
        var formatRx = (request.server.settings.state.cookies.strictHeader ? internals.parseRx.strict : internals.parseRx.loose);
