...
exports = module.exports = internals.Core = class {
    ...
    _registerLogger() {
        ...
        // The .default() doesn't support a full copy of the process.stdout instance
        if (!stdout) {
        ...
        let logger = new Log.Stdlogger(stdout, stderr);
        if (type === 'pino') {
            logger = new Log.Pinologger(stdout);
        }

        Log.plugin.register(this.root, { level, logger, additionalFields });
    }
...
internals.setup = function (options = {}) {
    ...
    let settings = Hoek.clone(options, { shallow: ['cache', 'listener', 'routes.bind', 'logger.stdout', 'logger.stderr'] });
