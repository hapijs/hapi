...
exports = module.exports = internals.Core = class {
    ...
    _initializeCache() {
        ...
        if (!this.cache.caches.has('_default')) {
    ...
    _createCache(configs) {
        ...
        for (let config of configs) {
            ...
            Hoek.assert(!this.cache.caches.has(name), 'Cannot configure the same cache more than once: ', name === '_default' ? 'default cache' : name);
            ...
            if (config.provider) {
                ...
                client = new Catbox.Client(provider.constructor, provider.options || { partition: 'hapi-cache' }, config.name);
            }
            else {
                client = new Catbox.Client(config.engine, null, config.name);
            }

            this.cache.caches.set(name, { client, segments: {}, shared: config.shared || false });
            added.push(client);
        }
    ...
    async _initialize() {
        ...
        try {
            const caches = [];
            this.cache.caches.forEach((cache) => caches.push(cache.client.start()));
    ...
    async _stop(options = {}) {
        ...
        try {
            ...
            const caches = [];
            this.cache.caches.forEach((cache) => caches.push(cache.client.stop()));
            await Promise.all(caches);
    ...
    _cachePolicy(options, _segment, realm) {
        ...
        const cache = this.cache.caches.get(cacheName);
