...
exports = module.exports = internals.Core = class {
    ...
    _initializeCache() {
        ...
        if (!this.caches.clients.has('_default')) {
    ...
    _createCache(configs) {
        ...
        for (let config of configs) {
            ...
            Hoek.assert(!this.caches.clients.has(name), 'Cannot configure the same cache more than once: ', name === '_default' ? 'default cache' : name);
            ...
            this.caches.clients.set(name, client);
            this.caches.public[name] = { segments: {}, shared: config.shared || false };
            added.push(client);
        }
    ...
    async _initialize() {
        ...
        try {
            const caches = [];
            this.caches.clients.forEach((client) => caches.push(client.start()));
    ...
    async _stop(options = {}) {
        ...
        try {
            ...
            const caches = [];
            this.caches.clients.forEach((client) => caches.push(client.stop()));
            await Promise.all(caches);
    ...
    _cachePolicy(options, _segment, realm) {
        ...
        const cache = this.caches.public[cacheName];
        ...
        const client = this.caches.clients.get(cacheName);
        const policy = new Catbox.Policy(options, client, segment);
        cache.segments[segment] = policy;
