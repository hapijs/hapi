...
internals.Ext.prototype.invoke = function (request, event, callback) {
    ...
    Async.forEachSeries(handlers, function (ext, next) {
        ...
        var bind = (request && ext.env ? ext.env.bind : null);
        if (request) {
            request._pluginEnv = ext.env;
        }

        internals.Ext.runProtected(request, event, next, function (enter, exit) {

            enter(function () {

                ext.func.call(bind, request || ext.env.root, exit);
            });
        });
    },
    function (err) {
...
internals.Ext.runProtected = function (logger, tags, next, setup) {        // setup: function (enter, exit)
    ...
    var finish = function () {
        ...
        if (isFinished) {
            if (logger) {
                logger.log(['hapi', 'duplicate', 'next', 'error'].concat(tags || []));
            }
