...
internals.Ext.runProtected = function (logger, tag, next, setup) {        // setup: function (enter, exit)
    ...
    var finish = function (/* arguments */) {
        ...
        if (isFinished) {
            if (logger) {
                logger.log(['hapi', tag, 'duplicate', 'next', 'error']);
            }
    ...
    var onError = function (err) {
        ...
        if (logger) {
            logger.log(['hapi', tag, 'uncaught', 'error'], err);                  // 'uncaught' treated special in request.log
        }
