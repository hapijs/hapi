...
internals.Generic.prototype._transmit = function (request, callback) {
    ...
    var prepare = function () {
        ...
        var isDeflate = acceptEncoding && acceptEncoding.indexOf('deflate') !== -1;

        if (isGzip && isDeflate) {
            isGzip = acceptEncoding.indexOf('gzip') < acceptEncoding.indexOf('deflate');
            isDeflate = acceptEncoding.indexOf('deflate') < acceptEncoding.indexOf('gzip');
        }
