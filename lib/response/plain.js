...
exports = module.exports = internals.Plain = function (source, request, options) {
    ...
    this._marshall = options.marshall || internals.marshall;
...
internals.marshall = function (response, callback) {
    ...
    if (response.source instanceof Stream) {
        response._payload = response.source;
        return callback();
    }

    var payload = response.source;
    if (response.settings.stringify) {
        var space = response.settings.stringify.space || response._request.server.settings.json.space;
        var replacer = response.settings.stringify.replacer || response._request.server.settings.json.replacer;
        var suffix = response.settings.stringify.suffix || response._request.server.settings.json.suffix || '';
        try {
    ...
    response._payload = new Payload(payload, response.settings);
    return callback();
};
