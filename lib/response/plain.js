...
exports = module.exports = internals.Plain = function (source, request, options) {
...
internals.Plain.prototype._marshall = function (callback) {

    var self = this;

    if (!this._processors.marshall) {
        return this._streamify(this.source, null, callback);
    }

    this._processors.marshall(this, function (err, source, options) {

        if (err) {
            return callback(err);
        }

        return self._streamify(source, options, callback);
    });
};


internals.Plain.prototype._streamify = function (source, options, callback) {
    ...
    var settings = options || this.settings;

    var payload = source;
    if (settings.stringify) {
        var space = settings.stringify.space || this.request.server.settings.json.space;
        var replacer = settings.stringify.replacer || this.request.server.settings.json.replacer;
        var suffix = settings.stringify.suffix || this.request.server.settings.json.suffix || '';
        try {
    ...
    this._payload = new Payload(payload, settings);
    return callback();
};
