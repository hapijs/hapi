...
exports._respond = function (item, request, callback) {
    ...
    var prepare = function (response) {
        ...
        if (!response ||
            ...
            response = Boom.badImplementation('Unexpected response item', response);
        }
        ...
        response._prepare(request, function (result) {
            ...
            if (!result._wasPrepared) {
                ...
                if (prepareCount > internals.maxNestedPreparations) {    // Prevent prepare loops
                    result = new internals.Error(Boom.badImplementation('Response prepare count exceeded maximum allowed', item), request.server.settings.json);
                    return send(result);
                }

                if (result.isBoom) {
                    if (errorPrepared) {
                        return send(new internals.Error(result, request.server.settings.json));
                    }
