...
NodeUtil.inherits(internals.Raw, Base);


internals.Raw.prototype._flush = function () {
    ...
    if (this._isDecorated) {
        ...
        if (error) {
            this._request.log(['http', 'response', 'direct', 'state'], error);
        }
...
internals.Raw.prototype._undecorate = function () {
