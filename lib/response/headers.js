...
exports.set = function (request, next) {
    ...
    if (response.settings.location) {
        response._header('location', exports.location(response.settings.location, request.connection, request));
    }
...
exports.location = function (uri, connection, request) {
    ...
    var baseUri = connection.settings.location || (connection.info.protocol + '://' + ((request && request.info.host) || (connection.info.host + ':' + connection.info.port)));
