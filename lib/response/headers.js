...
exports.location = function (uri, server, request) {
    ...
    var baseUri = server.settings.location || ((request && request.headers ? request.headers['x-forwarded-proto'] || server.info.protocol : server.info.protocol) + '://' + ((request && request.info.host) || (server.info.host + ':' + server.info.port)));
