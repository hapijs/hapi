...
exports.set = function (response, request) {
    ...
    if (response.header &&
        ...
        var isClientCached = (request._route && request._route.cache.isMode('client'));
        response.header('Cache-Control', (isClientCached && response._ttl) ? 'max-age=' + Math.floor(response._ttl / 1000) + ', must-revalidate' : 'no-cache');
