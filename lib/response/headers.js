...
exports.apply = function (response, request, next) {
    ...
    if (response._payload.size) {
        response._header('content-length', response._payload.size(response._flags.encoding));
    }

    if (response._flags.location) {
        response._header('location', internals.location(response._flags.location, request));
    }
...
internals.content = function (response, request) {
    ...
    if (type &&
        ...
        if (!hasParams ||
            ...
            response.type(type + (hasParams ? ', ' : '; ') + 'charset=' + (response._flags.charset || 'utf-8'));
        }
    }
};
...
internals.auth = function (response, request, next) {
    ...
    Auth.responseHeader(request, response, next);
};
