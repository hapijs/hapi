...
exports.location = function (uri, server, request) {
    ...
    var baseUri;
    if (server.settings.trustProxy) {
        baseUri = server.settings.location || ((request && request.headers ? request.headers['x-forwarded-proto'] || server.info.protocol : server.info.protocol) + '://' + ((request && request.info.host) || (server.info.host + ':' + server.info.port)));
    }
    else {
        baseUri = server.settings.location || (server.info.protocol + '://' + ((request && request.info.host) || (server.info.host + ':' + server.info.port)));
    }
...
internals.state = function (response, request, next) {
    ...
    Async.forEach(keys, function (name, nextKey) {
