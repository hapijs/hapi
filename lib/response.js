...
internals.Response.prototype._streamify = function (source, next) {
    ...
    if (this.variety === 'plain' &&
        ...
        var space = options.space || this.request.route.json.space;
        var replacer = options.replacer || this.request.route.json.replacer;
        var suffix = options.suffix || this.request.route.json.suffix || '';
        try {
