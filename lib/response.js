...
internals.Response.prototype._setSource = function (source) {
    ...
    if (source === null ||
        ...
        if (!this.statusCode &&
            source.statusCode) {                            // Stream is an HTTP response
...
internals.Response.prototype._marshall = function (next) {

    var self = this;

    this.statusCode = this.statusCode || 200;                       // When _prepare() is not called
...
internals.Response.prototype._streamify = function (source, next) {
    ...
    if (this.variety === 'plain' &&
        ...
        var space = options.space || this.request.connection.settings.json.space;
        var replacer = options.replacer || this.request.connection.settings.json.replacer;
        var suffix = options.suffix || this.request.connection.settings.json.suffix || '';
        try {
