...
exports.handler = function (route, options) {
    ...
    var handler = function (request, reply) {
        ...
        Async.forEachSeries(paths, function (path, next) {
            ...
            // Construct path if default extension set
            if (settings.defaultExtension) {

                Utils.assert(settings.defaultExtension.indexOf('.') !== 0, 'defaultExtension setting string should not lead with a period');
                ...
                var tryExtension = [fileName, settings.defaultExtension].join('.');
                var defaultExtensionPath = Path.join(path, tryExtension);
            }
            ...
            File.load(path, {}, null, request, function (err, response) {
                ...
                if (err.output.statusCode === 404) {
                    ...
                    // Try default extension after 404
                    if (defaultExtensionPath && selection.indexOf('.') === -1) {

                        File.load(defaultExtensionPath, {}, null, request, function (err, defaultExtensionResponse) {

                            if (!err) {
                                return reply(defaultExtensionResponse);
                            }
