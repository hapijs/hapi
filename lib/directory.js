...
exports.handler = function (route, options) {
    ...
    var absolutePath = File.absolutePath(route);
    ...
    var handler = function (request, reply) {
        ...
        var resource = decodeURIComponent(request.path);
        ...
        Async.forEachSeries(paths, function (path, next) {
            ...
            (new Response.File(path))._prepare(request, function (response) {
                ...
                if (settings.redirectToSlash &&
                    ...
                    return reply(new Response.Redirection(resource + '/'));
                }

                if (!index) {
                    return internals.generateListing(path, resource, selection, hasTrailingSlash, settings, reply);
                }

                var indexFile = Path.join(path, 'index.html');
                (new Response.File(indexFile))._prepare(request, function (indexResponse) {
...
internals.generateListing = function (path, resource, selection, hasTrailingSlash, settings, reply) {
