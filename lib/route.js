        ...
        if (param) {
            ...
            if (isMulti) {
                if (multiCount) {
                    for (var m = 0; m < multiCount; ++m) {
                        fingers.push('?');
                        this._segments.push({ isParam: true, name: name });
                    }
                }
                else {
                    fingers.push('#');
                    this._segments.push({ isParam: true, name: name, isWildcard: true });
                }
            }
            else {
                fingers.push('?');
                this._segments.push({ isParam: true, name: name, isOptional: isOptional });
            }
        }
        else {
            ...
            if (segment) {
                ...
                if (this.server.settings.router.isCaseSensitive) {
                    ...
                    fingers.push(segmentLower);
                    this._segments.push({ literal: segmentLower });
                }
            }
            else {
                pathRX += '\\/';
...
internals.Route.prototype.match = function (request) {
    ...
    var match = this._test(request.path);
    if (match) {
        request.params = {};
        ...
        if (this.params.length > 0) {
            for (var i = 1, il = match.length; i < il; ++i) {
                ...
                if (key) {
                    var decoded = internals.decodeURIComponent(match[i]);
                    if (decoded === null) {
                        match = null;
                        break;
                    }
                    ...
                    request._paramsArray.push(decoded);
                }
            }
        }
    }
...
internals.Route.prototype._test = function (path) {
...
exports.sort = function (a, b) {
    ...
    for (var i = 0; i < size; ++i) {
        ...
        if ((aSegment.isWildcard && bSegment.isWildcard) ||
            (aSegment.isParam && bSegment.isParam) ||
