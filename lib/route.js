...
*
* Path regular expression code adapted from Express, Copyright (c) 2009-2012 TJ Holowaychuk <tj@vision-media.ca>
...
exports = module.exports = Route = function (config, server) {
    ...
    this.server = server;
    this.config = (this.server.routeDefaults ? Utils.merge(Utils.clone(this.server.routeDefaults), config) : config);
    this.keys = [];
    this.cache = null;
    ...
    if (this.config.method === 'head') {

        Utils.abort('Cannot add a HEAD route');
    }
    ...
    if (this.config.authentication !== 'none' &&
        this.server.settings.authentication === null) {
    ...
    if (this.config.cache) {
        ...
        if (this.config.method !== 'get') {
            ...
            Utils.abort('Only GET routes can use a cache');
        }
...
Route.prototype.handler = function () {
    ...
    return function (request, next) {
        ...
        var call = function () {
            ...
            self.config.handler(request, function (result, options) {
...
Route.prototype.finalize = function () {
    ...
    return function (request, next) {
        ...
        self.server.emit('response', request.raw.req, res);
