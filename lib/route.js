    ...
    for (var i = 1, il = segments.length; i < il; ++i) {                            // Skip first empty segment
        ...
        if (param) {
            ...
            if (isMulti) {
                ...
                var extractRegex = new RegExp('^' + Utils.escapeRegex(pre) + '(.' + (isEmptyOk ? '*' : '?') + ')' + Utils.escapeRegex(post) + '$');
                this._segments.push({ name: name, isEmptyOk: isEmptyOk, pre: pre, post: post, extract: extractRegex, mixed: (!!pre || !!post) });
            }
        }
        else {
...
internals.compareMixed = function (aSegment, bSegment, p) {
    ...
    if (ap) {
        if (bp) {
            if (ap === bp) {
                if (p <= 2) {
                    return internals.compareMixed(p + 1);
                }
                else {
                    return 0;
                }
