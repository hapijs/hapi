...
exports = module.exports = Route = function (options, server) {
    ...
    if (this.config.auth.mode !== 'none') {
        ...
        config.auth.scope = config.auth.scope || null;
        config.auth.tos = config.auth.tos || this.server.settings.authentication.tos.min;
        config.auth.user = config.auth.user || 'required';
    }
