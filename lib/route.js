...
internals.state = function (request, next) {
    ...
    request.server.states.parse(cookies, (err, state, failed) => {
...
internals.payload = async function (request) {
    ...
    try {
        ...
        await new Promise((resolve, reject) => {
            ...
            const reply = request.server._replier.interface(request, request.route.realm, {}, Promises.split(resolve, reject));
...
internals.flush = async function (request) {

    // Flush out any pending request payload not consumed due to errors

    await new Promise((resolve) => Streams.drain(request.raw.req, resolve));
