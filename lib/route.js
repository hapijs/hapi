...
exports = module.exports = internals.Route = function (options, server, env) {
    ...
    this.settings.auth = (typeof this.settings.auth === 'string' ? { strategy: this.settings.auth } : this.settings.auth) || false;
    var auth = this.settings.auth;
    if (auth) {
        auth.mode = auth.mode || 'required';
        Utils.assert(['required', 'optional', 'try'].indexOf(auth.mode) !== -1, 'Unknown authentication mode: ' + auth.mode);

        Utils.assert(!auth.entity || ['user', 'app', 'any'].indexOf(auth.entity) !== -1, 'Unknown authentication entity type: ' + auth.entity);
        Utils.assert(!(auth.strategy && auth.strategies), 'Route can only have a auth.strategy or auth.strategies (or use the default) but not both');
        auth.strategies = auth.strategies || [auth.strategy || 'default'];
        delete auth.strategy;

        auth.payload = auth.payload || 'none';
        var hasAuthenticatePayload = false;
        auth.strategies.forEach(function (strategy) {

            Utils.assert(self.server.auth.strategies[strategy], 'Unknown authentication strategy: ' + strategy);
            hasAuthenticatePayload = hasAuthenticatePayload || typeof self.server.auth.strategies[strategy].authenticatePayload === 'function';
        ...
        Utils.assert(auth.payload === 'none' || hasAuthenticatePayload, 'Payload authentication requires at least one strategy with payload support');
    }
