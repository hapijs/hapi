...
exports = module.exports = Route = function (options, server) {
    ...
    this.cache = new Cache.Policy('url', this.config.cache, this.server.cache);
    ...
    if (this.config.pre) {
        for (var i = 0, il = this.config.pre.length; i < il; ++i) {
            ...
            if (typeof pre.method === 'string') {
                ...
                var helper = this.server.helpers[pre];
                Utils.assert(helper, 'Unknown prerequisite server helper method: ' + pre);
                pre.method = helper;
            }
