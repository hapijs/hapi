...
exports = module.exports = Route = function (options, server) {
    ...
    if (this.path instanceof RegExp) {
        ...
        var pathRx = this.path.concat(self.server.settings.router.isStrict ? '' : '/?')
            .replace(/\/\(/g, '(?:/')
...
Route.prototype.finalize = function (request, next) {
    ...
    if (request.response.result) {
        ...
        if (request.response.error.type === 'oauth') {
            exports.respond(request, request.response.error.code, { error: request.response.error.error, error_description: request.response.error.text });
        }
        else {
            exports.respond(request, request.response.error.code, { error: request.response.error.text, message: request.response.error.message, code: request.response.error.code });
        }

        request.log(['http', 'response', 'error'], request.response.error);
    }
    else {
        exports.respond(request, 200);
    }
