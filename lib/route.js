...
exports = module.exports = internals.Route = function (options, server) {
    ...
    */
    ...
    else if ((typeof this.config.handler === 'object') && !!this.config.handler.docs) {
        if (typeof this.config.handler.docs === 'object') {
           this.config.handler.docs.docsEndpoint = this.path;
        }
        else {
            this.config.handler.docs = {
                docsEndpoint: this.path
            };
        }

        var docsEndpoint = Docs.endpoint(this.config.handler.docs);
        this.config.handler = docsEndpoint.handler;
        this.config.query = docsEndpoint.query;
    }
