...
internals.state = function (request, next) {
    ...
    request.connection._stateDefinitions.parse(cookies, function (err, state, failed) {
        ...
        if (!err ||
            request.route.state.failAction === 'ignore') {

            return next();
        }

        request._log(['state', 'error'], { header: cookies, errors: err.data });
        return next(request.route.state.failAction === 'error' ? err : null);
    });
};
