...
exports = module.exports = internals.Route = function (options, connection, realm) {
    ...
    options = Schema.assert('route', options, options.path);
    ...
    this.settings = Schema.assert('routeConfig', this.settings, options.path);
...
internals.Route.prototype.rebuild = function () {
    ...
    if (this.connection._extensions.onPreAuth) {
        cycle.push(this.connection._extensions.onPreAuth);
    }
    ...
    if (this.connection._extensions.onPostAuth) {
        cycle.push(this.connection._extensions.onPostAuth);
    }
    ...
    if (this.connection._extensions.onPreHandler) {
        cycle.push(this.connection._extensions.onPreHandler);
    }

    cycle.push(Handler.execute);                                     // Must not call next() with an Error

    if (this.connection._extensions.onPostHandler) {
        cycle.push(this.connection._extensions.onPostHandler);       // An error from here on will override any result set in handler()
    }
    ...
    this._onPreResponse = this.connection._extensions.onPreResponse;
    this._cycle = cycle;
};
