...
exports = module.exports = internals.Route = function (options, server, env) {
    ...
    if (env &&
        ...
        options = Utils.cloneWithShallow(options, ['config']);
        options.path = (env.route.prefix || '') + options.path;
    ...
    Hoek.assert(internals.Route.pathRegex.validatePath.test(options.path), 'Invalid path:', options.path);
    Hoek.assert(options.path.match(internals.Route.pathRegex.validatePathEncoded) === null, 'Path cannot contain encoded non-reserved path characters:', options.path);
    ...
    this.settings = Utils.cloneWithShallow(options.config, ['bind', 'plugins', 'app']) || {};
    ...
    if (this.settings.cache) {
        Hoek.assert(this.method === 'get', 'Only GET routes can use a cache:', options.path);
...
internals.Route.prototype._parsePath = function () {
    ...
    for (var i = 1, il = segments.length; i < il; ++i) {                            // Skip first empty segment
        ...
        if (param) {
            ...
            Hoek.assert(!params[name], 'Cannot repeat the same parameter name:', name, 'in:', this.settings.path);
            params[name] = true;
            ...
            if (isMulti) {
                ...
                if (pre || post) {
                    ...
                    segmentMeta.extract = new RegExp('^' + Hoek.escapeRegex(pre) + '(.' + (isEmptyOk ? '*' : '+') + ')' + Hoek.escapeRegex(post) + '$', (!isCaseSensitive ? 'i' : ''));
                }
