...
exports = module.exports = internals.Route = function (options, server) {
    ...
    Utils.assert((typeof this.config.handler === 'function') ^ !!this.config.handler.proxy ^ !!this.config.handler.file ^ !!this.config.handler.directory, 'Handler must be a function or an object with a proxy or an object with a file or directory');
    ...
    if (typeof this.server.auth == 'undefined') {
        if (this.server.settings.strategies) {
            if (Object.keys(this.server.settings.strategies).length == 0){
                // Called from Shot.inject()
                // May need to do addt'l work, depending on unit test results
            }
            else {
                Utils.assert(!!this.server.settings.strategies.default && this.config.auth.mode !== 'none', "Route has no default authentication strategy to fallback on");
            }
        }
    }
    ...
    */
    ...
    else if ((typeof this.config.handler === 'object') && !!this.config.handler.directory) {
