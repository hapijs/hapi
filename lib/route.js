...
internals.state = async function (request) {
    ...
    let parseError;
    try {
        result = await request._core.states.parse(cookies);
    }
    catch (err) {
        parseError = err;
    }
    ...
    return request._core.toolkit.failAction(request, request.route.settings.state.failAction, parseError, ['state', 'error'], { log: { header: cookies, errors: parseError.data } });
};
...
internals.payload = async function (request) {
    ...
    try {
        ...
        return request._core.toolkit.failAction(request, request.route.settings.payload.failAction, err, ['payload', 'error']);
    }
};
