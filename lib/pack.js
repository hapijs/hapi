...
internals.Pack.prototype._register = function (plugin, permissions, options, callback, _dependencies) {
    ...
    var step = function (labels, subset) {
        ...
        if (permissions.ext) {
            methods.ext = function () {
                ...
                var extOptions = arguments[2] || {};
                extOptions.context = extOptions.context || self._env[plugin.name].context;

                self._applySync(selection.servers, Server.prototype._ext, [arguments[0], arguments[1], extOptions, plugin.name]);
            };
        }
    ...
    root.context = function (context) {
        ...
        self._env[plugin.name].context = context;
    };
