...
exports = module.exports = internals.Pack = function (options) {
    ...
    this._methods = {};                                                 // Method functions
...
internals.Pack.prototype._register = function (plugin, options, callback, _dependencies) {
    ...
    root.method = function (/* name, method, options */) {
        ...
        return self._method.apply(self, arguments);
    };

    root.helper = function (/* name, method, options */) {

        return self._helper.apply(self, arguments);
    };
...
internals.Pack.prototype._method = function (/* name, fn, options | {} | [{}, {}] */) {

    var items = [].concat(typeof arguments[0] === 'string' ? { name: arguments[0], fn: arguments[1], options: arguments[2] } : arguments[0]);
    for (var i = 0, il = items.length; i < il; ++i) {
        var item = items[i];
        this._addMethod(item.name, item.fn, item.options);
    }
};


internals.Pack.prototype._addMethod = function (name, fn, options) {
    ...
    Utils.assert(!this._methods[name] && !this._helpers[name], 'Helper or method function name already exists');
    ...
    var cache = null;
    if (settings.cache) {
        cache = this._provisionCache(settings.cache, 'method', name, settings.cache.segment);
    }
    ...
    var method = function (/* arguments, methodNext */) {
        ...
        var generateFunc = function (next) {
            ...
            args[lastArgPos] = next;
            fn.apply(null, args);
        };

        var key = settings.generateKey.apply(null, args);
        if (key === null) {                             // Value can be ''
            self.log(['hapi', 'method', 'key', 'error'], { name: name, args: args });
        }
...
internals.Pack.prototype._addHelper = function (name, method, options) {
