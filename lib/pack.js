...
internals.Pack.prototype.allow = function (permissions) {
    ...
    var scoped = {
        register: function (name, options, callback) {
            ...
            self._register(name, rights, callback ? options : {}, callback || options);
        },
        require: function (name, options, callback) {

            self._require(name, rights, callback ? options : {}, callback || options);
        }
    };
...
internals.getSourceFilePath = function () {
    ...
    // 1 - internals.Pack._require
    // 2 - internals.Pack.require
    // 3 - **Caller

    var stack = Utils.callStack();
    return stack[3][0].substring(0, stack[3][0].lastIndexOf('/'));
};
