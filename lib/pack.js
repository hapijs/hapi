...
internals.Pack.prototype._register = function (plugin, options, callback, _dependencies) {
    ...
    Hoek.assert(plugin, 'Missing plugin');
    Hoek.assert(callback, 'Missing callback');
    Hoek.assert(!this._env[plugin.name], 'Plugin already registered:', plugin.name);
    Hoek.assert(plugin.name, 'Plugin missing name');
    Hoek.assert(plugin.name !== '?', 'Plugin name cannot be \'?\'');
    Hoek.assert(plugin.version, 'Plugin missing version');
    Hoek.assert(plugin.register && typeof plugin.register === 'function', 'Plugin missing register() method');
    ...
    root.loader = function (requireFunc) {
        ...
        Hoek.assert(!requireFunc || typeof requireFunc === 'function', 'Argument must be null or valid function');
    ...
    root.views = function (options) {
        ...
        env.views = new Views.Manager(options);
    };
    ...
    plugin.register.call(null, root, options || {}, function (err) {
        ...
        if (!_dependencies &&
            ...
            Hoek.assert(!dependencies[plugin.name].length, 'Plugin', plugin.name, 'missing dependencies:', dependencies[plugin.name].join(', '));
        }
...
internals.Pack.prototype._select = function (labels, subset) {
    ...
    Hoek.assert(!labels || Array.isArray(labels), 'Bad labels object type (undefined or array required)');
...
internals.Pack.prototype._require = function (name, options, callback, requireFunc) {
    ...
    Hoek.assert(name && (typeof name === 'string' || typeof name === 'object'), 'Invalid plugin name(s) object: must be string, object, or array');
    Hoek.assert(!options || typeof name === 'string', 'Cannot provide options in a multi-plugin operation');
...
internals.Pack.compose = function (manifest /* [options], callback */) {
    ...
    Hoek.assert(manifest.servers && manifest.servers.length, 'Pack missing servers definition');
    Hoek.assert(manifest.plugins, 'Pack missing plugins definition');
    Hoek.assert(typeof callback === 'function', 'Invalid callback');

    var pack = new internals.Pack(Hoek.applyToDefaults(options, manifest.pack || {}));
    ...
    pack.require(manifest.plugins, function (err) {
