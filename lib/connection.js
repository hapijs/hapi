...
exports = module.exports = internals.Connection = function (server, options) {
    ...
    Hoek.assert(this.type === 'port' || this.settings.port === undefined, 'Cannot specify port with a UNIX domain socket or a Windows named pipe');
    Hoek.assert(this.settings.autoListen || this.settings.port === undefined, 'Cannot specify port when autoListen is false');
    Hoek.assert(this.settings.autoListen || this.type === 'port', 'Cannot specify a UNIX domain socket or a Windows named pipe when autoListen is false');

    this._host = (host ? (this.type === 'unix' ? Path.resolve(host) : (this.type === 'windows' ? host : host.toLowerCase())) : '');
    this._port = this.settings.port || 0;
...
internals.Connection.prototype._start = function (callback) {
    ...
    if (this.type !== 'port') {
        this.listener.listen(this._host, callback);
    }
    else {
        this.listener.listen(this._port, this._host, callback);
    }
};
