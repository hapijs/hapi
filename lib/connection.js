...
internals.Connection.prototype._dispatch = function (options) {
    ...
    return function (req, res) {
        ...
        const request = self.server._requestor.request(self, req, res, options);

        // Check load

        const overload = self._load.check();
        if (overload) {
...
internals.Connection.prototype._route = function (configs, plugin) {
    ...
    for (let i = 0, il = configs.length; i < il; ++i) {
        const config = configs[i];

        if (Array.isArray(config.method)) {
            for (let m = 0, ml = config.method.length; m < ml; ++m) {
                const method = config.method[m];
...
internals.Connection.prototype._addRoute = function (config, plugin) {
    ...
    for (let i = 0, il = vhosts.length; i < il; ++i) {
