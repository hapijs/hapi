...
exports = module.exports = internals.Connection = function (host, port, options, pack) {
    ...
    this._load = this.pack._core._heavy.policy(this.settings.load);
...
internals.Connection.prototype._dispatch = function (options) {
    ...
    return function (req, res) {
        ...
        if (!self._load.check()) {
            self.log(['hapi', 'load'], self.pack.load);
            request._reply(Boom.serverTimeout('Server under heavy load', self.pack.load));
        }
        else {
...
internals.Connection.prototype._ext = function (event, func, options) {
