...
exports = module.exports = internals.Manager = class {
    ...
    decorate(property, method) {
        ...
        Hoek.assert(['abandon', 'authenticated', 'close', 'continue', 'entity', 'redirect', 'wrap', 'realm', 'request', 'state', 'unauthenticated', 'unstate'].indexOf(property) === -1, 'Cannot override built-in toolkit decoration:', property);
...
/*
    const handler = function (request, h) {
        ...
        result / h.wrap(result)             -> result               // Not allowed in auth or in ext before handler
        h.wrap(result).takeover()           -> result (respond)
        h.continue                          -> null                 // Defaults to null only in handler and pre, not allowed in auth

        throw error / h.wrap(error)         -> error (respond)      // failAction override in pre
