...
exports = module.exports = internals.Plugin = function (pack, servers, env, options) {
    ...
    if (this !== this._pack) {
        ...
        this.events = new Kilt(this.servers, this._pack._events);
    }

    this.auth = {
        scheme: function () {

            internals.applyChildSync(self.servers, 'auth', 'scheme', arguments);
        },
        strategy: function () {

            internals.applyChildSync(self.servers, 'auth', 'strategy', arguments);
        }
    };
};
...
internals.Plugin.prototype._select = function (labels, name, options) {
    ...
    if (labels) {
        ...
        if (!name &&
            connections.length === this.servers.length) {
...
internals.Plugin.prototype.expose = function (/* key, value */) {
    ...
    internals.expose(this.servers, this._env.name, arguments);              // server.plugins

    if (this.servers.length === this._pack.servers.length) {
...
internals.Plugin.prototype.route = function (options) {
    ...
    internals.applySync(this.servers, Server.prototype._route, [options, this._env]);
};


internals.Plugin.prototype.state = function () {

    internals.applySync(this.servers, Server.prototype.state, arguments);
};


internals.Plugin.prototype.ext = function () {

    internals.applySync(this.servers, Server.prototype._ext, [arguments[0], arguments[1], arguments[2], this._env]);
};
...
internals.Plugin.prototype.dependency = function (deps, after) {
    ...
    deps = [].concat(deps);
    this._pack._dependencies.push({ plugin: this._env.name, servers: this.servers, deps: deps });
...
internals.Plugin.prototype.register = function (plugins /*, [options], callback */) {
    ...
    this._pack._register(plugins, options, this, callback);
};
