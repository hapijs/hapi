...
internals.Plugin.prototype.register = function (plugins /*, [options], callback */) {
    ...
    */
    ...
    for (var i = 0, il = plugins.length; i < il; ++i) {
        ...
        registration.dependencies = Schema.assert('dependencies', registration.dependencies, 'must be a string or an array of strings');
    ...
    Items.serial(registrations, function (item, next) {
        ...
        var selection = self._select(options.select, item.name);
        selection.realm.modifiers.route.prefix = options.routes && options.routes.prefix;
        selection.realm.modifiers.route.vhost = options.routes && options.routes.vhost;
        selection.realm.pluginOptions = item.options || {};
