    ...
    */
    ...
    for (var i = 0, il = plugins.length; i < il; ++i) {
        ...
        Hoek.assert(typeof item.dependencies === 'string' ||
            ...
            item.dependencies.every(internals.isString),
            'Dependencies must be a string or an array of strings');
    ...
    Items.serial(registrations, function (item, next) {
        ...
        if (item.dependencies.length) {
            ...
            selection.dependency(item.dependencies, function (server, next) {
                ...
                item.register(server, item.options || {}, next);
            });
...
internals.isString = function (str) {
