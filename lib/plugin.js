...
exports = module.exports = internals.Plugin = function (server, connections, env, parent) {         // env can be a realm or plugin name
    ...
    Podium.call(this, [connections, server._events]);
...
internals.Plugin.prototype.encoder = function (encoding, compressor) {

    this._apply('encoder', Connection.prototype.encoder, [encoding, compressor]);
};
...
internals.Plugin.prototype.log = function (tags, data, timestamp, _internal) {
    ...
    this.root._events.emit({ name: 'log', tags: tagsMap }, event);
