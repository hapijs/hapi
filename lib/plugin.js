...
exports = module.exports = internals.Plugin = function (server, connections, env, options) {
    ...
    this.auth = {
        default: function () { self._applyChild('auth', 'default', arguments); },
        scheme: function () { self._applyChild('auth', 'scheme', arguments); },
        strategy: function () { self._applyChild('auth', 'strategy', arguments); },
...
internals.Plugin.prototype.register = function (plugins /*, [options], callback */) {
    ...
    /*
        ...
        plugins = plugin, items, [plugin, item]
    */
    ...
    for (var i = 0, il = plugins.length; i < il; ++i) {
        ...
        var pluginOptions = plugin.plugin && plugin.options;
        plugin = plugin.plugin || plugin;

        Hoek.assert(typeof plugin.register === 'function', 'Plugin register must be a function', hint);
...
internals.Plugin.prototype.after = function (func, after) {

    this._server._afters = this._server._afters || new Topo();
    this._server._afters.add({ func: func, plugin: this }, { after: after, group: this._env.plugin });
};
...
internals.Plugin.prototype.dependency = function (deps, after) {
    ...
    deps = [].concat(deps);
    this._server._dependencies.push({ plugin: this._env.plugin, connections: this.connections, deps: deps });

    if (after) {
        this.after(after, deps);
    }
};
...
internals.Plugin.prototype.expose = function (/* key, value */) {
    ...
    if (this.connections.length === this._server.connections.length) {
        internals.expose([this._server], this._env.plugin, arguments);                    // server.plugins
    }
};
...
internals.Plugin.prototype.ext = function () {
    ...
    this._apply(Connection.prototype._ext, [arguments[0], arguments[1], arguments[2], this._env]);
};
...
internals.Plugin.prototype.state = function () {
    ...
    this._apply(Connection.prototype._state, arguments);
};
...
internals.Plugin.prototype.views = function (options) {
    ...
    if (!options.relativeTo &&
        this._env.relativeTo) {

        options = Hoek.shallow(options);
        options.relativeTo = this._env.relativeTo;
    }
