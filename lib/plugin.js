...
exports = module.exports = internals.Plugin = function (root, connections, env, options) {

    var self = this;

    Kilt.call(this, connections, root._events);
    ...
    this.events = this;                                                                 // Backwards compatibility
    this.app = this._core.app;
    this.plugins = this._core.plugins;
    this.load = this._core._heavy.load;
    this.auth = this._core._auth;
    this.settings = this._core._settings;
    ...
    this.auth = {
        ...
        default: function () {
            ...
            internals.applyChildSync(self.connections, 'auth', 'default', arguments);
        }
    };
};
...
internals.Plugin.prototype.expose = function (/* key, value */) {
    ...
    if (this.connections.length === this._core.connections.length) {
        internals.expose([this._core], this._env.plugin, arguments);                    // server.plugins
    }
};
...
internals.Plugin.prototype.state = function () {
    ...
    internals.applySync(this.connections, Connection.prototype._state, arguments);
};
...
internals.Plugin.prototype.log = function (tags, data, timestamp) {
    ...
    this._core._events.emit('log', event, tagsMap);
