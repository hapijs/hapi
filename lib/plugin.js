...
internals.Plugin.prototype._select = function (labels, plugin, options) {
    ...
    return new internals.Plugin(this._pack, connections, plugin || this._env, options);
};
...
internals.Plugin.prototype.expose = function (/* key, value */) {
    ...
    internals.expose(this.connections, this._env.plugin, arguments);              // server.plugins

    if (this.connections.length === this._pack.connections.length) {
        internals.expose([this._pack], this._env.plugin, arguments);          // pack.plugins
    }
};
...
internals.Plugin.prototype.dependency = function (deps, after) {
    ...
    deps = [].concat(deps);
    this._pack._dependencies.push({ plugin: this._env.plugin, connections: this.connections, deps: deps });
...
internals.Plugin.prototype._after = function (func, after) {
    ...
    this._pack._afters.add({ func: func, plugin: this }, { after: after, group: this._env.plugin });
};
...
internals.Plugin.prototype.cache = function (/* name, options */) {
    ...
    if (this._env.plugin) {
        return this._pack._provisionCache(options, 'plugin', this._env.plugin, options.segment);
    }
