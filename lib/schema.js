...
exports.routeHandler = function (config, handlers) {

    var schema = Utils.clone(internals.routeHandlerSchema);
    var objectSchema = Utils.clone(internals.routeHandlerObjectSchema);
    var error;

    // Extend the schema based on user defined handlers
    for (var key in handlers) {
        if (handlers[key].builtin !== true) {
            objectSchema[key] = Joi.any();
        }
    }

    schema.push(Joi.object(objectSchema).length(1));
...
internals.routeHandlerObjectSchema = {
