...
internals.cache = internals.cacheBase.concat(Joi.object({
...
internals.cacheManifest = internals.cacheBase.concat(Joi.object({
    engine: Joi.alternatives([
        Joi.object(),
        Joi.string(),
        Joi.func()
...
internals.server = internals.serverBase.concat(Joi.object({
...
internals.pack = internals.packBase.concat(Joi.object({
    cache: Joi.alternatives([
        Joi.func(),
        internals.cache,
        Joi.array().includes(internals.cache).min(1)
...
internals.manifest = Joi.object({
    pack: internals.packBase.concat(Joi.object({
        cache: Joi.alternatives([
            Joi.string(),
    ...
    servers: Joi.array().required().includes(internals.serverManifest).min(1),
    plugins: Joi.object()
});
