...
exports.route = function (options, config, next) {
    ...
    Joi.validate(options, internals.routeOptionsSchema, function (err) {

        if (err) {
            return next(err.annotated());
        }

        Joi.validate(config, internals.routeConfigSchema, function (err) {

            return next(err ? err.annotated() : null);
        });
    });
};
