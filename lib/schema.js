...
exports.routeConfig = function (config) {
    ...
    if (typeof config.handler === 'object') {

        var handlerError = Joi.validate(config.handler, internals.routeHandlerSchema);
        return (handlerError ? handlerError.annotated() : null);
    }
...
internals.routeConfigSchema = {
	...
	// Commented out for issue 1385 to allow for overriding of default directory,file,proxy,views handlers 
        /*Joi.object({
...
exports.addHandlerSchema = function (name, schema) {

    Utils.assert(typeof name === 'string', 'Not a valid handler name');
    Utils.assert(typeof schema === 'object', 'Not a valid schema object');
    internals.routeHandlerSchema[name] = schema;
    console.log(internals.routeHandlerSchema);
};
