...
// Check if variables exist & are not null
...
// Check settings to see if enough data provided to send email

exports.unableToEmail = function(settings) {

    // Check that email object exists
    if (this.exists(settings, settings.email)) {

        // Check host data
        var hostkeys = ['host', 'user', 'password'];
        for(var i in hostkeys) {

            console.log(hostkeys[i], settings.email.server[hostkeys[i]], this.exists(settings.email.server[hostkeys[i]]))
            if (!this.exists(settings.email.server[hostkeys[i]])) {

                return "Process.settings.email.server has missing key (" + hostkeys[i]  + ")";
            }
        }
        
        // Check address data
        var addrkeys = ['fromName', 'replyTo'];
        for(var j in addrkeys) {

            if (!this.exists(settings.email[addrkeys[i]])) {

                return "Process.settings.email has missing key (" + addrkeys[j] + ")";
            }
        }
        ...
        return "Process.settings.email does not exist";
    }
}
...
exports.email = function (to, subject, text, html, callback) {
    ...
    var emailerr = this.unableToEmail(Process.settings);
    if (emailerr === null) {
        ...
        if (callback) {
            ...
            callback(Err.internal(emailerr));
        }
        else {
