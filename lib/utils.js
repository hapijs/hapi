...
// Check if variable exists, non-null
...
exports._canEmail = function(settings) {

    // TODO: convert to data based approach for more specific error messages
    var exists = this.exists;

    var hasHostdata = this.exists(
        settings.email, 
        settings.email.server, 
        settings.email.server.host,
        settings.email.server.user,
        settings.email.server.password);

    var hasAddressData = this.exists(
        settings.email.fromName,
        settings.email.replyTo);
    
    if (!hasHostdata) {

        Log.err("No email credentials supplied.");
    }
    
    if (!hasAddressData) {

        Log.err("No email address data supplied.");
    }
    
    return (hasHostdata && hasAddressData);
}
...
exports.email = function (to, subject, text, html, callback) {
    ...
    if (this._canEmail(Process.settings)) {
        ...
        var err = "No email credentials supplied.";
        if (callback) {

            callback(Err.internal(err));
        }
        else {
