...
exports = module.exports = internals.Proxy = function (options) {
    ...
    this.settings.port = this.settings.port || (this.settings.isHttps ? 443 : 80);
...
internals.Proxy.prototype.handler = function () {
    ...
    return function (request) {
        ...
        var options = {
            url: (self.settings.isHttps ? 'https' : 'http') + '://' + self.settings.host + ':' + self.settings.port + request.path,
        ...
        if (request.method === 'get' ||
            ...
            var call = Request(options, function (err, response, payload) {
            ...
            if (request._route &&
                request._route.settings.payload === 'stream') {

                request.raw.req.pipe(stream);
            }
