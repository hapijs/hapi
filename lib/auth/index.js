...
exports = module.exports = internals.Auth = function (server, options) {
    ...
    if (options.strategies) {
        this.setStrategies(options.strategies);
    ...
    this.scheme = this.loadScheme(options);
...
internals.Auth.prototype.setStrategies = function (strategies) {
    ...
    var schemes = Object.keys(strategies);
    Utils.assert(schemes.length > 0, 'Number of Authentication Strategies must be greater than zero');
...
internals.Auth.prototype.authenticate = function (request, next) {
    ...
    if (config.strategy) {
        if (!this.strategies.hasOwnProperty(config.strategy)) {
            request.log(['auth', 'error'], 'Invalid auth strategy defined in request: ' + JSON.stringify(request));
            return validate(Err.forbidden('Invalid auth strategy defined in request: ' + config.strategy));
        }
        else {
            var schemeObj = this.strategies[config.strategy];
            var scheme = this.loadScheme(schemeObj);
