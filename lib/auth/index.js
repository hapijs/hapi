...
exports = module.exports = internals.Auth = function (server, options) {
    ...
    for (var name in settings) {
        if (settings.hasOwnProperty(name)) {
            ...
            Utils.assert(['oz', 'basic', 'hawk', 'cookie'].indexOf(strategy.scheme) !== -1 || strategy.scheme.indexOf('ext:') === 0, name + ' has an unknown scheme: ' + strategy.scheme);
...
internals.Auth.prototype.authenticate = function (request, next) {
    ...
    var validate = function (err, session, wasLogged) {
        ...
        request.session = session;
        request.session.strategy = self.strategies[config.strategies[strategyPos - 1]];
...
internals.Auth.validatePayload = function (request, next) {
    ...
    else if (config.payload === 'optional' && !request.session.hash) {
        return next();
    }

    if (request.session.strategy.validatePayload(request.rawBody, request.session)) {
        return next();
    }
