...
internals.Auth.prototype.add = function (name, options) {
    ...
    Utils.assert(options.scheme, name + ' is missing a scheme');
    Utils.assert(['oz', 'basic', 'hawk', 'cookie', 'bewit'].indexOf(options.scheme) !== -1 || options.scheme.indexOf('ext:') === 0, name + ' has an unknown scheme: ' + options.scheme);
    Utils.assert(options.scheme.indexOf('ext:') !== 0 || options.implementation, name + ' has extension scheme missing implementation');
    ...
    switch (options.scheme) {
        case 'oz': this.strategies[name] = new Oz(this.server, options); break;
        case 'hawk': this.strategies[name] = new Hawk(this.server, options); break;
        case 'basic': this.strategies[name] = new Basic(this.server, options); break;
        case 'cookie': this.strategies[name] = new Cookie(this.server, options); break;
        case 'bewit': this.strategies[name] = new Bewit(this.server, options); break;
        default: this.strategies[name] = options.implementation; break;
    }
...
internals.Auth.prototype.loadConfig = function (options) {
