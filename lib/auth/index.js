...
internals.Auth.prototype.authenticate = function (request, next) {
    ...
    var validate = function (err, credentials, options) {
        ...
        if (err) {
            if (options.log) {
                request._log(options.log.tags, options.log.data);
            }
            else {
        ...
        if (config.scope &&
            ...
            request._log(['auth', 'error', 'scope'], { got: credentials.scope, need: config.scope });
        ...
        if (tos &&
            ...
            request._log(['auth', 'error', 'tos'], { min: tos, received: credentials.tos });
