...
ArrayType.prototype._includes = function() {
    ...
    return function(values, qs, key, req) {
        ...
        if (valueIsValid === false && args[0]._checks.indexOf("includes") < 0) {
            ...
            req.addValidationError("the value(s) `" + invalidValues + "` in array `" + key + "` must be " + args[0].type + "(s)")
        }
        return valueIsValid;
    }
}
...
ArrayType.prototype._excludes = function() {
    ...
    return function(value, qs, key, req) {
        ...
        console.log('hmm1', result, req.validationErrors)
        if (result === false) {
            ...
            console.log('hmm2', req.validationErrors)
        }
        return result;
    }
}
