...
BaseType.prototype.increment = function(key){
  // Convert to array so indices match with INTERNAL_DATA_KEY array
...
BaseType.prototype._rename = function(to, deletingOriginal, allowingMultiple){
  var self = this;
  self.renamed = self.renamed || []
  deletingOriginal = deletingOriginal || false
  allowingMultiple = allowingMultiple || false
  
  return function(value, qstr, key){
    if (allowingMultiple === false && to in self.renamed){
      return false;
    }
    
    qstr[to] = value;
    if (deletingOriginal === true){
      delete qstr[key]
    }
    
    if (allowingMultiple === false){
      self.renamed.push(to)
    }
...
BaseType.prototype.rename = function(to, deletingOriginal, allowingMultiple){
  this.add("rename", this._rename(to, deletingOriginal, allowingMultiple))
  return this;
}
