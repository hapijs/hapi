...
internals.parse = function (result, headers, callback) {
    ...
    // Ensure consistent tick callback

    var callbackTick = function (err, obj) {
    ...
    if (mime === 'application/json') {
        ...
        return callbackTick(error, obj);
    }

    // Form-encoded

    if (mime === 'application/x-www-form-urlencoded') {
        return callbackTick(null, Querystring.parse(result));
    }
    ...
    return callbackTick(Boom.badRequest('Unsupported content-type'));
};
