...
exports.read = function (request, next) {
    ...
    if (clientTimeout && request._timestamp) {
        clientTimeout -= Date.now() - request._timestamp;
        clientTimeoutId = setTimeout(internals.clientTimeoutError.bind(finish), clientTimeout);
    }
...
internals.clientTimeoutError = function () {

    this(Err.clientTimeout('Client is taking too long to send request'));
};