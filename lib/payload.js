...
exports.read = function (request, next) {
    ...
    var mime = (contentType ? contentType.split(';')[0] : 'application/json');          //  Defaults to 'application/json'
    var parserFunc = null;
    ...
    req.on('end', function () {
        ...
        var unzip = function () {
            ...
            Zlib.unzip(new Buffer(payload, encoding), function (err, buffer) {
                ...
                if (err) {
                    return parse(Err.badRequest('Invalid gzip: ' + err));
                }

                var unzipped = buffer.toString();
                request.rawBody = unzipped;
                return parse(null, unzipped);
            });
        };
