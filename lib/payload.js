...
exports.read = function (request, next) {
    ...
    if (level === 'stream') {
        Utils.assert(!(request.raw.req instanceof Injection._Request), 'Cannot use injection with stream payload');
        return next();
    }

    // Injection

    if (request.raw.req instanceof Injection._Request) {

        request.rawBody = request.raw.req._payload;
        if (level === 'parse') {
            request.payload = {};

            if (request.raw.req._payload) {
                try {
                    request.payload = parserFunc(request.raw.req._payload);
                }
                catch (err) {
