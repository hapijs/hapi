...
exports.replyInterface = function (request, finalize, base) {
    ...
    var viewsManager = (base && base.env && base.env.views) || request._route._env.views || request.server._views;
    if (viewsManager) {
        reply.view = function (template, context, options) {

            return internals.wrap(viewsManager.response(template, context, options, request, Response.Plain), request, finalize);
        };
    }
...
internals.wrap = function (result, request, finalize) {
    ...
    var prepare = function () {
        ...
        if (response._prepare) {
            return response._prepare(response, finalize);
        }
...
exports.defaults = function (method, handler, server) {
    ...
    if (typeof handler === 'object') {
        ...
        var serverHandler = server.pack._handlers[type];
