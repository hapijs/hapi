...
exports.response = function (result, request, onSend) {
    ...
    var response = (result instanceof Error ? (result.isBoom ? result : new Boom(result))
                                            : (result instanceof Response.Generic ? result : new Response.Generic(result)));
    ...
    return function (request, next) {
        ...
        Ext.runProtected(request, 'pre', next, function (enter, exit) {
            ...
            var finalize = function (result) {
                ...
                if (pre.assign) {
                    request.pre[pre.assign] = result.hasOwnProperty('source') ? result.source : result;
