...
exports.execute = async function (request) {
    ...
    let result = internals.handler(request, request.route.settings.handler);
...
internals.handler = async function (request, method, pre) {
    ...
    const response = await request._core.responder.execute(method, request, { bind, realm, continue: 'null' });
