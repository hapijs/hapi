...
exports.configure = function (handler, route) {
    ...
    if (typeof handler === 'object') {
        ...
        var serverHandler = route.server.pack._handlers.handlers[type];
        var configHandler = handler[type];
        var schemaError;

        Utils.assert(serverHandler, 'Unknown handler:', type);
        schemaError = Joi.validate(configHandler, serverHandler.schema);
        Utils.assert(!schemaError, 'Invalid handler options for', type, ':', schemaError);

        return serverHandler.fn(route, configHandler);
    }
...
exports.registerDefaultHandlers = function (server) {
    ...
    server.handler('proxy', Proxy.handler, Schema.proxyHandlerSchema);
    server.handler('file', File.handler, Schema.fileHandlerSchema);
    server.handler('directory', Directory.handler, Schema.directoryHandlerSchema);
    server.handler('view', Views.handler, Schema.viewHandlerSchema);
};
