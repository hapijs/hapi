...
exports.configure = function (handler, route) {
    ...
    if (typeof handler === 'object') {
        ...
        var handlerKey = Object.keys(handler)[0]; // Get name of handler
        if (handlerKey in handlerTypes) {

            return handlerTypes[handlerKey](route, handler[handlerKey]); // Return value of handler function
        }
    }
        ...
        enter(function () {
            ...
            Async.forEachSeries(handlers, function (ext, next) {
                ...
                var filter = function (result) {
                    ...
                    return next(result && result.source !== null ? result : null);
                };
...
exports.registerHandler = function (name, method) {

    Utils.assert(typeof name === 'string', 'Not a valid handler name: ' + name);
    Utils.assert(typeof method === 'function', 'Not a valid method parameter for handler: ' + name);
    handlerTypes[name] = method;
};


// Add "default" handlers to handlerTypes
exports.registerHandler("directory", Directory.handler);
exports.registerHandler("proxy", Proxy.handler);
exports.registerHandler("file", File.handler);
exports.registerHandler("view", Views.handler);
