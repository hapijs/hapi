...
exports.configure = function (handler, route) {
    ...
    Utils.assert((typeof handler === 'function') ^ !!handler.proxy ^ !!handler.file ^ !!handler.directory ^ !!handler.view ^ (handler === 'notFound'), 'Handler must be a function or equal notFound or be an object with a proxy, file, directory, or view:', route.settings.path);

    if (handler === 'notFound') {
        return internals.notFound();
    }

    if (typeof handler === 'object') {
        if (handler.proxy) {
            route.proxy = handler.proxy;
            return Proxy.handler(handler.proxy, route);
        }

        if (handler.file) {
            return Files.fileHandler(route, handler.file);
        }

        if (handler.directory) {
            return Files.directoryHandler(route, handler.directory);
        }
    ...
    return function (request, next) {
        ...
        Ext.runProtected(request, 'pre', next, function (enter, exit) {
            ...
            var finalize = function (result) {
                ...
                return exit(null, result, !result.toCache);
