...
exports.execute = function (request, next) {
    ...
    const finalize = function (err, result) {
...
internals.handler = function (request, callback) {
    ...
    const finalize = function (response, data) {
...
exports.prerequisites = function (config, server) {
    ...
    */
    ...
    for (let i = 0, il = config.length; i < il; ++i) {
        const pres = [].concat(config[i]);

        const set = [];
        for (let p = 0, pl = pres.length; p < pl; ++p) {
            let pre = pres[p];
...
internals.fromString = function (type, notation, server) {
    ...
    result.method = function (request, reply) {
        ...
        const finalize = function (err, value, cached, report) {
        ...
        const args = [];
        for (let i = 0, il = methodArgs.length; i < il; ++i) {
