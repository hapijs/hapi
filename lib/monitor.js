...
/**
 * Middleware (early in stack) required for others to work
 */
...
Monitor.prototype.onPreHandler = function() {
  var public_methods = ["uptime", "memory", "cpu"]
  ...
  var wrapped = function(req, res, next) {
    ...
    for(var i in public_methods) {
      req[public_methods[i]] = this[public_methods[i]];
    }
...
Monitor.prototype.onPostHandler = function() {
  ...
  // var interval = this.options.outbound.interval;
 ...
 *
 * @api public
 * @param digits number of decimal places to keep, default 2
 */ 
 ...
 *
 * @api public
 * @param callback function to process result
 */
 ...
 *
 * @api private 
 * @param callback function to process the asynchronous result
 */
