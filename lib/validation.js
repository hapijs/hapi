...
internals.input = function (source, key, request, next) {
    ...
    var err = exports.lib.validate(request[key], request.route.validate[source]);
    if (!err) {
        return next();
    }
...
exports.response = function (request, next) {
    ...
    var error = exports.lib.validate(request._response.raw, request.route.response.schema);
    if (!error) {
        return next();
    }
