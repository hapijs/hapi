...
internals.input = function (source, request, next) {
    ...
    const postValidate = (err, value) => {
        ...
        request.server._replier.execute(request.route.settings.validate.failAction, request, { realm: request.route.realm, args: [source, error] }).then(next);
    };
    ...
    return schema.call(bind, request[source], localOptions, postValidate);
};
...
exports.response = function (request, next) {
    ...
    const postValidate = (err, value) => {
        ...
        request.server._replier.execute(request.route.settings.response.failAction, request, { realm: request.route.realm, args: [err] }).then(next);
    };
    ...
    return schema(source, localOptions, postValidate);
};
