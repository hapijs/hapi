...
exports.validateQuery = function(req, parameters, callback){
    ...
    for(var i in params){
        ...
        for(var j in validators){
            var validatorName = parameters[key]._checks[j];
            console.log("validator: ", validatorName)
            if (validatorName in mutatorMethods){
                finalizeFns.push(j);
                continue;
            }
            ...
            var result = validators[j](value, req.hapi.query, key);
            // console.log("validator #" + i + "." + j, key, value, typeof value, result)
            if (result === false){
                isInvalid = true;
                invalidParam = key + " = " + value;
                break;
            }
            //end section
        }
    ...
    if (isInvalid) {
        ...
        callback(Err.badRequest('Invalid parameter: ' + invalidParam));
    }
    else {
