...
exports.compile = function (rule, compiler, realm, core) {
    ...
    Hoek.assert(compiler, 'Cannot set uncompiled validation rules without configuring a validator');
    const validator = compiler.compile(rule);

    Hoek.assert(typeof validator?.validate === 'function', 'Compilation result is not a valid validator. Missing validate() method');
    return validator;
};
