...
exports = module.exports = internals.Composer = function (manifest, criteria) {

    this._settings = Utils.clone(manifest);
    this._criteria = Utils.clone(criteria);

    if (Array.isArray(this._settings) === false) {
...
internals.Composer.prototype.compose = function (callback) {
    ...
    var settings = [];
    for (var i = 0, il = this._settings.length; i < il; ++i) {
        var set = this._settings[i];
        var store = new Confidence.Store();
        var err = store.load(set);
        if (err) {
            return callbackTick(err);
        }

        store.get('/', this._criteria, function (err, value) {          // Sync API
