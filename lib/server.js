...
module.exports = internals.Server = function (/* host, port, options */) {        // all optional
    ...
    if (this.settings.monitor) {
        this._monitor = new HapiLog.Monitor(this);
        HapiLog.log.event(['info', 'config'], this.settings.nickname + ': Monitoring enabled');
    }
    ...
    if (this.settings.cache) {
        ...
        HapiLog.log.event(['info', 'config'], this.settings.nickname + ': Caching enabled');
    }
    else {
        this.cache = null;
    }
    ...
    if (this.settings.debug) {
        ...
        HapiLog.log.event(['info', 'config'], this.settings.nickname + ': Debug console enabled');
    }
...
internals.Server.prototype.start = function (callback) {
    ...
    this.listener.once('listening', function () {
        ...
        HapiLog.log.event('info', self.settings.nickname + ': Instance started at ' + self.settings.uri);
...
internals.Server.prototype.stop = function () {
    ...
    this.listener.close();
    HapiLog.log.event('info', this.settings.nickname + ': Instance stopped at ' + this.settings.uri);
};
...
internals.Server.prototype.addRoute = function (options) {
    ...
    for (var i = 0, il = routes.length; i < il; ++i) {
        Utils.assert(route.fingerprint !== routes[i].fingerprint, 'New route: ' + route.path + ' conflicts with existing: ' + routes[i].path);
    }
...
internals.Server.prototype.addHelper = function (name, method, options) {
    ...
    var log = function (tags, data) {
        ...
        return HapiLog.log.event(tags, data);
    };
