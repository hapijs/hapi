...
module.exports = internals.Server = function (/* host, port, options */) {        // all optional
    ...
    if (args.pack) {
        Utils.assert(!this.settings.cache, 'Cannot configure server cache in a pack member');
...
internals.Server.prototype.stop = function (callback) {
    ...
    this.plugin.stop(callback);
};


internals.Server.prototype._stop = function (callback) {
...
internals.Server.prototype.helper = internals.Server.prototype.addHelper = function (name, method, options) {
    ...
    if (settings.cache) {
        ...
        cache = new Catbox.Policy(settings.cache, this.plugin._cache);
    }
