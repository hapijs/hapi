...
internals.Core = function (options, server) {

    options = options || {};

    Schema.assert('server', options);

    this._settings = Hoek.applyToDefaultsWithShallow(internals.defaults, options, ['app', 'plugins']);

    this._root = server;
    this._caches = {};                                                  // Cache clients
    this._methods = new Methods(server);                                // Server methods
...
internals.Server.prototype.connection = function (/* host, port, options */) {
    ...
    if (args.options) {
        Hoek.assert(!args.options.cache, 'Cannot configure cache when adding a connection to a server');
    }
