...
module.exports = internals.Server = function (/* host, port, options */) {        // all optional
    ...
    if (this.settings.cache) {
        this.cache = new Cache.Client(this.settings.host + '/' + this.settings.port, this.settings.cache);
...
internals.Server.prototype.addHelper = function (name, method, options) {
    ...
    Utils.assert(typeof method === 'string', 'method must be a function');
    Utils.assert(typeof name === 'string', 'name must be a string');
    Utils.assert(!options || typeof options === 'object', 'options must be a string');

    options = options || {};

    // Create cache

    var cache = (options.cache ? new Cache.Policy('helper:' + name, options.cache, this.cache) : null);
    var log = Log.event;

    var helper = function (/* arguments, next */) {

        var lastArg = arguments.length - 1;
        var next = arguments[lastArg];
        delete arguments[lastArg];

        var key = (cache ? internals.generateKey(arguments) : null);

        if (cache &&
            key === null) {                             // Value can be ''

            log(['helper', 'key', 'error'], { name: name, args: arguments });
        }
