...
internals.Server.prototype.inject = function (options, callback) {
    ...
    var shotOptions = Hoek.clone(options); // options can be reused
    var requestOptions = (shotOptions.credentials ? { credentials: shotOptions.credentials } : null);

    delete shotOptions.credentials;

    var needle = this._dispatch(requestOptions);
    Shot.inject(needle, shotOptions, function (res) {
