...
internals.Server.prototype.ext = function (event, shouldAppend, func) {
    ...
    if (this._ext.hasOwnProperty(event) && this._ext[event]) {
        ...
        if (shouldAppend) {
            ...
            this._ext[event] = func.concat(this._ext[event]);
        }
    }
    else {
        this._ext[event] = [func];
    }
};
