...
internals.Server = class {
    ...
    async register(plugins, options = {}) {
        ...
        const registrations = [].concat(plugins).map((item) => {
            ...
            */
            ...
            const registration = {
                register: item.plugin.register,
                name: item.plugin.name || item.plugin.pkg.name,
                version: item.plugin.version || item.plugin.pkg.version,
                multiple: item.plugin.multiple,
                pluginOptions: item.options,
                dependencies: item.plugin.dependencies,
                options: {
                    once: item.plugin.once || (item.once !== undefined ? item.once : options.once),
                    routes: {
                        prefix: item.routes.prefix || options.routes.prefix,
                        vhost: item.routes.vhost || options.routes.vhost
                    }
                }
            };
