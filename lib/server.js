...
module.exports = internals.Server = function (/* host, port, options */) {        // all optional
    ...
    // if host contains a `/`, then host is a path to a unix domain socket
    if (args.host && args.host.indexOf('/') !== -1) {
      this._unixDomainSocket = Path.resolve(args.host);  // path to unix domain socket
    } else if (args.host && args.host.indexOf('\\\\.\\pipe') !== -1 ) {
        this._namedPipe = args.host;
    } else { // hostname or IP
    ...
    */
    ...
    if (this._unixDomainSocket) {
        this.info = { unixDomainSocket: this._unixDomainSocket };
    } else if (this._namedPipe) {
        this.info = { namedPipe: this._namedPipe };
    } else {
        this.info = {
...
internals.Server.prototype._start = function (callback) {
    ...
    this.listener.once('listening', function () {
        ...
        if (self._unixDomainSocket) {
            self.info.unixDomainSocket = address; // address is string path
        } else if (self._namedPipe) {
            self.info.namedPipe = address; // address is a string path
        } else { // TCP
    ...
    if (this._unixDomainSocket) { // listen to unix domain socket
        this.listener.listen(this._unixDomainSocket);
    } else if (this._namedPipe) {  
        this.listener.listen(this._namedPipe);
    } else { // TCP
