...
internals.Server.prototype._start = function (callback) {
    ...
    if (this.settings.load.eventLoopSampleInterval) {
        var loopSample = function () {

            var start = Date.now();
            var measure = function () {

                if (self.load.eventLoopDelaySamples.length >= self.settings.load.eventLoopSampleSize) {
                    self.load.eventLoopDelaySamples.shift();
                }

                self.load.eventLoopDelaySamples.push(Date.now() - start - self.settings.load.eventLoopSampleInterval);
                self.load.eventLoopDelay = internals.avg(self.load.eventLoopDelaySamples);
                loopSample();
            };

            self._eventLoopTimer = setTimeout(measure, self.settings.load.eventLoopSampleInterval);
        };
