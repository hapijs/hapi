...
internals.Server.prototype._unlisten = function (options) {
    ...
    const timeout = () => {
        ...
        Object.keys(this._sockets).forEach((key) => this._sockets[key].destroy());
        this._sockets = {};
    };
...
internals.Server.prototype._dispatch = function (options) {
    ...
    return (req, res) => {
        ...
        if (req.socket) {
            ...
            res._hapiDispatcher = { server: this, req };
            res.on('finish', internals.onFinish);
        }
...
internals.onFinish = function () {
    ...
    const { server, req } = this._hapiDispatcher;
    this._hapiDispatcher = null;
...
internals.Server.prototype._initializeListener = function () {
    ...
    this._listener.once('listening', () => {
        ...
        this._onConnection = (connection) => {
            ...
            connection._hapiConnection = { server: this, key };
            connection.on('close', internals.onClose);
        };
...
internals.onClose = function () {
    ...
    const { server, key } = this._hapiConnection;
    this._hapiConnection = null;
    delete server._sockets[key];
};
