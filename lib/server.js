...
module.exports = internals.Server = function (/* host, port, options */) {        // all optional
    ...
    Utils.assert(this.settings.load.sampleInterval || (!self.settings.load.maxEventLoopDelay && !self.settings.load.maxHeapUsedBytes && !self.settings.load.maxRssBytes), 'Load sample interval must be set in enable load limits');
    ...
    */
    ...
    if (this.settings.payload.multipart &&
        typeof this.settings.payload.multipart === 'string') {
...
internals.Server.prototype._dispatch = function (options) {
    ...
    return function (req, res) {
        ...
        if ((self.settings.load.maxEventLoopDelay && self.load.eventLoopDelay > self.settings.load.maxEventLoopDelay) ||
            (self.settings.load.maxHeapUsedBytes && self.load.heapUsed > self.settings.load.maxHeapUsedBytes) ||
            (self.settings.load.maxRssBytes && self.load.rss > self.settings.load.maxRssBytes)) {

            return request._reply(Boom.serverTimeout());
        }
    ...
    if (this.settings.load.sampleInterval) {
        var loopSample = function () {
            ...
            var measure = function () {
                ...
                if (self.load.eventLoopDelaySamples.length >= self.settings.load.sampleSize) {
                ...
                if (self.load.heapUsedSamples.length >= self.settings.load.sampleSize) {
                    self.load.heapUsedSamples.shift();
                }

                self.load.heapUsedSamples.push(mem.heapUsed);
                self.load.heapUsed = internals.avg(self.load.heapUsedSamples);

                // Heap used

                if (self.load.rssSamples.length >= self.settings.load.sampleSize) {
                    self.load.rssSamples.shift();
                }

                self.load.rssSamples.push(mem.rss);
                self.load.rss = internals.avg(self.load.rssSamples);
