...
exports.Server = Server = function (host, port, options, routes) {
    ...
    var listenerEntryFunc = function (req, res) {
        ...
        internals.executeExtensions(that.settings.ext.onRequest, req, res, function () {
...
Server.prototype.unhandledRoute = function () {
    ...
    return function (next) {
        ...
        if (that.settings.ext.onUnknownRoute) {
            ...
            if (that.settings.ext.onUnknownRoute instanceof Array) {

                Utils.abort('ext.onUnknownRoute cannot be an array');
            }
...
Server.prototype.addRoute = function (config) {
    ...
    var inWrapper = function (func) {

        return function () {

            var args = Array.prototype.slice.call(arguments);   // Convert arguments to instance of Array
    ...
    var extWrapper = function (func) {

        return function () {

            var args = Array.prototype.slice.call(arguments);   // Convert arguments to instance of Array
            var next = args[args.length - 1];
            internals.executeExtensions(func, this.req, this.res, next);
        };
    };

    // Build route chain

    var chain = [inWrapper(this.preRoute(routeConfig))];

    chain.push(inWrapper(this.routeValidator(routeConfig)));

    if (this.settings.ext.onPreHandler) {

        chain.push(extWrapper(this.settings.ext.onPreHandler));
    }

    chain.push(inWrapper(this.routeHandler(routeConfig)));

    if (this.settings.ext.onPostHandler) {

        chain.push(extWrapper(this.settings.ext.onPostHandler));
    }

    chain.push(inWrapper(this.postRoute()));

    if (this.settings.ext.onPostRoute) {

        chain.push(extWrapper(this.settings.ext.onPostRoute));
    }
...
internals.executeExtensions = function (ext, req, res, next) {

    if (ext) {

        var extFuncs = (ext instanceof Array ? ext : [ext]);
        Async.forEachSeries(extFuncs, function (item, callback) {
