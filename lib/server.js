...
exports = module.exports = internals.Server = function (options) {
    ...
    if (options.cache) {
        ...
        for (let i = 0, il = caches.length; i < il; ++i) {
...
internals.Server.prototype.connection = function (options) {
    ...
    for (let i = 0, il = registrations.length; i < il; ++i) {
...
internals.Server.prototype.initialize = function (callback) {
    ...
    for (let i = 0, il = this._dependencies.length; i < il; ++i) {
        const dependency = this._dependencies[i];
        if (dependency.connections) {
            for (let s = 0, sl = dependency.connections.length; s < sl; ++s) {
                const connection = dependency.connections[s];
                for (let d = 0, dl = dependency.deps.length; d < dl; ++d) {
                    const dep = dependency.deps[d];
    ...
    const caches = Object.keys(self._caches);
