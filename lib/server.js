...
internals.Server.prototype._dispatch = function (options) {
    ...
    return function (req, res) {
        ...
        request._onRequestExt(self.settings.ext.onRequest, function () {
            ...
            if (routes) {
                for (var ri = request.path.split('/').length - 1, rl = routes.length; ri < rl; ++ri) {

                    var currentRoutes = routes[ri];

                    if (currentRoutes) {
                        for (var i = 0, il = currentRoutes.length; i < il; ++i) {
...
internals.Server.prototype._match = function (method, path) {
    ...
    if (routes) {
        for (var ri = path.split('/').length - 1, rl = routes.length; ri < rl; ++ri) {
...
internals.Server.prototype._routeTable = function () {
    ...
    Object.keys(this._routes).forEach(function (method) {
        ...
        self._routes[method].forEach(function (methodRoutes) {
...
internals.Server.prototype.addRoute = function (options) {
    ...
    var segmentCount = route.path.split('/').length;

    routes[segmentCount] = routes[segmentCount] || [];

    for (var ri = 0, rl = routes.length; ri < rl; ++ri) {
        routes[ri] = routes[ri] || [];
        for (var i = 0, il = routes[ri].length; i < il; ++i) {
            Utils.assert(route.fingerprint.trim() && route.fingerprint !== routes[ri][i].fingerprint, 'New route: ' + route.path + ' conflicts with existing: ' + routes[ri][i].path);
        }
    }

    routes[segmentCount].push(route);

    routes[segmentCount].sort(function (a, b) {

        return a.fingerprint && (a.fingerprint.length < b.fingerprint.length) ? 1 : 0;
    });
