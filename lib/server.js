...
exports = module.exports = internals.Server = function (/* host, port, options */) {        // all optional
    ...
    for (var a = 0, al = arguments.length; a < al; ++a) {
        ...
        Utils.assert(!args[key], 'Bad server constructor arguments: duplicated arg type:', type, '(values: `' + args[key] + '`, `' + argVal + '`)');
        args[key] = argVal;
    }
    ...
    */
    ...
    this.helpers = this.pack._methods.helpers;              // Helper functions
    ...
    if (this.settings.security) {
        if (this.settings.security.hsts) {
            if (this.settings.security.hsts === true) {
                this.settings.security._hsts = 'maxAge=15768000';
            } else if (typeof this.settings.security.hsts === 'number') {
                this.settings.security._hsts = 'maxAge=' + this.settings.security.hsts;
            } else {
                this.settings.security._hsts = 'maxAge=' + this.settings.security.hsts.maxAge || 15768000;
...
internals.Server.prototype.helper = function () {

    return this.pack._helper.apply(this.pack, arguments);
};
