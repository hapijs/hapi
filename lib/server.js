...
exports.Server = Server = function (host, port, options, routes) {
    ...
    if (this.settings.monitor) {
        ...
        if (this.settings.monitor.ops &&
            ...
            this.on('ops', that.monitor.handle('ops'));
        ...
        if (this.settings.monitor.request &&
            ...
            this.on('request', that.monitor.handle('request'));
            this.on('replied', function(hapi) {

                // that.emit('request', hapi.request, hapi.response);
                // console.log('replied')
                that.emit('request', hapi.request, hapi.response)
            })
        }
    ...
    if (this.settings.tls) {
        ...
        this.listener = Https.createServer(tls, listernerEntryFunc);
    }
    else {

        this.listener = Http.createServer(listernerEntryFunc);
    }
...
Server.prototype.unhandledRoute = function () {
    ...
    return function (next) {
        ...
        if (that.settings.ext.onUnknownRoute) {
            ...
            that.settings.ext.onUnknownRoute(this.req, this.res);
            next();
        }
        else {
...
Server.prototype.addRoute = function (config) {
    ...
    this.router[config.method.toLowerCase()](config.path, { stream: true }, [

        wrapper(this.settings.ext.onPreRoute),
        wrapper(this.preRoute()),
        wrapper(this.routeValidator(config)),
        wrapper(this.settings.ext.onPreHandler),
        wrapper(this.routeHandler(config)),
