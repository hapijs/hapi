...
internals.Server.prototype._stop = function (options, callback) {
    ...
    var connectionKeys = Object.keys(self._connections);

    var checkConnections = function () {

        self._started = false;
        self.listener.close(callback);

        if (connectionKeys.length) {
            return setTimeout(destroy, options.timeout);
        }
...
internals.Server.prototype._destroy = function () {
