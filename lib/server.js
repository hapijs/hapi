...
internals.Server.prototype.plugin = function (config, options) {
    ...
    Utils.assert(!this._plugin || (!config && !options), 'Cannot configure server plugin interface more than once');
    ...
    this._plugin = new Pack(config, options);
    this._plugin.addServer('default', this);
    return this._plugin;
};
