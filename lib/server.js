...
Server.prototype.addRoute = function (options) {
    ...
    Utils.assert(!!routeOptions.handler ^ !!routeOptions.config.handler, 'Handler must appear once and only once');     // XOR
...
Server.prototype.respond = function (request, code, payload, headers) {
    ...
    request._raw.res.writeHeader(code, headers);
    request._raw.res.end(request.method !== 'head' ? data : '');
};
