...
module.exports = internals.Server = function (/* host, port, options */) {        // all optional
    ...
    Utils.assert(this.settings.name.match(/|/g) === null, 'Server name includes an illegal charactger (|)');
    ...
    if (this.settings.cache) {
        this.cache = new Cache.Client(this);
...
internals.Server.prototype.addHelper = function (name, method, options) {
    ...
    var cache = new Cache.Policy('helper/' + name, settings.cache, this.cache);
