...
internals.Server.prototype.initialize = async function () {
    ...
    const caches = Object.keys(this._caches).map((name) => {

        return new Promise((resolve, reject) => this._caches[name].client.start(Promises.split(resolve, reject)));
    });
...
internals.Server.prototype._invoke = function (type) {
