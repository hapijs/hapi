...
exports = module.exports = internals.Server = function (/* host, port, options */) {        // all optional
    ...
    */
    ...
    this._notfound.fingerprint = '/#';
    this._notfound.params = ['p'];
...
internals.Server.prototype._route = function (configs, env) {
    ...
    [].concat(configs).forEach(function (config) {
        ...
        methods.forEach(function (method) {
            ...
            var route = new Route(config, self, env);                // Do no use config beyond this point, use route members
            var vhosts = [].concat(route.settings.vhost || '*');

            vhosts.forEach(function (vhost) {

                var record = self._router.add({ method: route.method, path: route.path, vhost: vhost, analysis: route._analysis }, route);
