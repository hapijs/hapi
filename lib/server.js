...
internals.Server.prototype.addRoute = function (options) {
    ...
    for (var ri = 0, rl = routes.length; ri < rl; ++ri) {
        Utils.assert(route.fingerprint.trim() && route.fingerprint !== routes[ri].fingerprint, 'New route: ' + route.path + ' conflicts with existing: ' + routes[ri].path);
    }
    ...
    routes.sort(function (a, b) {
        ...
        var aParts = a.path.split('/').length;
        var bParts = b.path.split('/').length;

        return (a.path[a.path.length - 1] === '}' && aParts === bParts) || aParts > bParts ? 1 : 0;
    });
