...
module.exports = internals.Server = function (/* host, port, options */) {        // all optional
    ...
    var host = args.host ? args.host.toLowerCase() : 'localhost';
    this._started = false;
    this.settings = Utils.applyToDefaults(Defaults.server, args.options);
    this.settings.host = args.host && args.host.toLowerCase();
    this.settings.port = typeof args.port !== 'undefined' ? args.port : 0;
    this.settings.nickname = host + ':' + this.settings.port;
    this.settings.uri = (this.settings.tls ? 'https://' : 'http://') + host + ':' + this.settings.port;
...
internals.Server.prototype.start = function (callback) {
    ...
    this.listener.once('listening', function () {
        ...
        // update the port, host, and uri with what was actually bound
    ...
    if (this.settings.host) {
        ...
        this.listener.listen(this.settings.port);
    }
};
