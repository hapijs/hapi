...
internals.Manager.prototype.render = function (filename, context, options) {
    ...
    var render = function () {
        ...
        if (!engine.config.layout) {
            ...
            try {
                return execute(engine, compiled, context, options)(context, options);
            }
            catch (err) {
        ...
        if (context &&
            context.hasOwnProperty(engine.config.layoutKeyword)) {
        ...
        try {
            layoutContext[engine.config.layoutKeyword] = execute(engine, compiled, context, options)(context);
            return execute(engine, layout, layoutContext, options)(layoutContext);
        }
        catch (err) {
            return Boom.internal(err.message, err);
        }
    ...
    var execute = function (engine, compiled, ctx, options) {

        if (engine &&
            typeof engine.module.execute === 'function') {

            return engine.module.execute(engine, compiled, ctx, options);
        }
