...
internals.Engines.prototype.add = function (settings, key) {
    ...
    if (typeof settings.module == 'string') {
        try {
            this._e[key] = require(settings.module);  // Can only require modules from hapi's package.json
        }
        catch (e) {
...
internals.Manager.prototype.loadEngines = function () {
    ...
    for(var i = 0, il = engines.length; i < il; ++i) {
        ...
        for(var j = 0, jl = baseMethods.length; j < jl; ++j) {
            ...
            if (!engine.hasOwnProperty(method) || !engine[method]) {
...
internals.Manager.prototype._isCaching = function (filename, globalSettings, engineSettings) {

    globalSettings = globalSettings || this.settings;
    engineSettings = engineSettings || Defaults.views.engines.html;
    
    if (!globalSettings.cache) {
        return false; // if global cache disabled, disable all caching behavior
    }
    
    if (!engineSettings.cache) {
...
internals.Manager.prototype.setCache = function (filename, compiled, options, engineSettings) {
    ...
    if (this._isCaching(filename, options, engineSettings)) {
...
internals.Manager.prototype._get = function (filename, options) {
    ...
    this.setCache(filename, compiled, engine.cache);
