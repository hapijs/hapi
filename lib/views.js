...
internals.Response.prototype._prepare = function (request, callback) {
    ...
    this.manager.render(this.template, this.context, this.options, function (err, rendered, config) {

        if (err) {
            return Utils.nextTick(callback)(err);
        }

        var response = new Response.Generic(rendered);
        if (config.contentType) {
            response._header('content-type', config.contentType);
        }
