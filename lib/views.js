...
internals.Manager.prototype.initEngine = function (ext, options) {
    ...
    this._engines[ext] = {}
    this._engines[ext].engine = require(options.module);
    
    var getSubFn = function(obj, fnName) {
        if (obj.hasOwnProperty(fnName)) {
            return obj[fnName];
        }
        else {
            if (obj.engine && obj.engine.hasOwnProperty(fnName)) {
                return obj.engine[fnName];
            }
            else {
                return null;
            }
    ...
    var methods = ['compile', 'registerPartial', 'registerHelper'];
    for(var i in methods) {
        var method = methods[i];
        this._engines[ext][method] = getSubFn(options, method);
    }
    
    
    // if (options.hasOwnProperty('compile')) {
    //     this._engines[ext].compile = options.compile;
    // }
    // else {
    //     this._engines[ext].compile = (function(engine){ return engine.compile; })(this._engines[ext].engine);
    // }
    
    // if (options.hasOwnProperty('registerPartial')) {
    //     this._engines[ext].compile = options.compile;
    // }
    // else {
    //     this._engines[ext].compile = (function(engine){ return engine.compile; })(this._engines[ext].engine);
    ...
    // Utils.assert(this._engines.hasOwnProperty(ext), 'Invalid engine configured for ' + ext);
    
    this._engines[ext].suffix = '.' + ext;
    
    console.log(ext, this._engines[ext]);
};
