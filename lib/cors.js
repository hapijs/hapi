...
exports.route = function (options) {
    ...
    if (settings.origin.indexOf('*') !== -1) {
        ...
        if (settings.wildcardIgnoresOrigin) {
            settings._ignoreOrigin = true;
        }
    }
    else {
        settings._origin = {
...
internals.handler = function (request, h) {
    ...
    if (!origin && !settings._ignoreOrigin) {
    ...
    const responseOrigin = settings._ignoreOrigin ? '*' : origin;
    response._header('access-control-allow-origin', responseOrigin);
...
exports.headers = function (request) {
    ...
    const origin = settings._ignoreOrigin ? '*' : request.headers.origin;
