...
exports.authenticate = function (request, config, next) {
    ...
    Mac.authenticate(request.raw.req, loadTokenFunc, { isHTTPS: request.server.settings.tls }, function (isAuthenticated, session, err) {

        if (!isAuthenticated) {

            // Unauthenticated

            if (config.auth.mode === 'optional' &&
        ...
        if (config.auth.scope &&
            !session.scope[config.auth.scope]) {

            return next(Err.forbidden('Insufficient token scope (\'' + config.auth.scope + '\' expected for client ' + session.client + ')'));
        }

        if (config.auth.entity === 'any') {

            // User Mode: any

            return next();
        }
        else if (config.auth.entity === 'user') {
            ...
            if (session.user) {
                ...
                if (config.auth.tos === 'none' ||
                    (session.tos && session.tos >= config.auth.tos)) {
        ...
        else if (config.auth.entity === 'client') {
...
exports.token = {
    schema: {
        ...
        client_secret: Types.String(),      // empty
    ...
    handler: function (request) {
        ...
        function getOrCreate(user, client, customResponseFields) {
            ...
            if (!user ||
                ...
                (request.session.scope && request.session.scope.authorized === true)) {
