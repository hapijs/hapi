...
//        exports.loadToken(request.server.settings.authentication.aes256Keys.oauthToken, token, callback);
    ...
    var validate = function (err, ticket, attributes) {
        ...
        if (request._route.config.auth.scope &&
            ...
            request.log(['auth', 'error'], 'Insufficient scope');
        ...
        request.log(['auth', 'error'], 'Unknown entity mode');
...
exports.token = {
    ...
    handler: function (request) {
        ...
        serverSettings.loadClientFunc(request.payload.client_id, function (err, client) {
            ...
            if (err || !client) {
                return request.reply(exports.error('invalid_client', 'Invalid client identifier or secret'));
            }
            ...
            if (request.payload.grant_type === 'client_credentials') {
                ...
                return getOrCreate(null, client);
            }
                ...
                if (refresh.client !== client._id) {
                    return request.reply(exports.error('invalid_grant', 'Mismatching refresh token client id'));
                }
                ...
                serverSettings.loadUserFunc(refresh.user, function (err, user) {
                    ...
                    return getOrCreate(user, client);
                });
            }
                ...
                serverSettings.extensionFunc(request, client, function (err, user, action) {
                    ...
                    return getOrCreate(user, client, action);
                });
            }
            else {
        ...
        function getOrCreate(user, client, customResponseFields) {
            ...
            if (!user ||
                (client.scope && client.scope.indexOf('authorized') !== -1) ||
            ...
            function issue() {
                ...
                Oz.Ticket.issue(ticketAttr.app, ticketAttr.user, ticketAttr.options, function (err, ticket) {
                    ...
                    if (user) {
                        ticket.refresh_token = exports.encrypt(serverSettings.aes256Keys.oauthRefresh, { user: user._id, client: client._id });
                    }
...
internals.parseClientCredentials = function (request) {
    ...
    if (authorization) {
        if (request.payload.client_id ||
            ...
            return exports.error('invalid_request', 'Request cannot include both Basic and payload client authentication');
        }
        ...
        if (parts.length === 2 &&
            ...
            return exports.error('invalid_request', 'Unsupported HTTP authentication scheme');
        }
    }
    else {
        ...
        if (request.payload.client_id) {
            ...
            return exports.error('invalid_request', 'Request missing client authentication');
        }
    }
};
