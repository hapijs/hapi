...
exports.authenticate = function (request, next) {
    ...
    Oz.Request.authenticate(request.raw.req, request.server.settings.authentication.encryptionPassword, { isHttps: request.server.settings.tls }, validate);
};
...
exports.token = {
    ...
    handler: function (request) {
        ...
        serverSettings.getAppFunc(request.payload.client_id, function (err, app) {
            ...
            if (request.payload.grant_type === 'client_credentials') {
                ...
                if (!request.payload.rsvp) {
                    return request.reply(new Oz.Error('invalid_request', 'Missing rsvp'));
                }
        ...
        function issue(user, app, customResponseFields) {
            ...
            var generate = function (rsvp) {
                ...
                Oz.Ticket.generate(ticketAttr.app, ticketAttr.user, request.server.settings.authentication.encryptionPassword, ticketAttr.options, function (err, ticket) {
