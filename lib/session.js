...
exports.token = function (req, reply) {
    ...
    var serverSettings = req.server.settings.authentication;
    ...
    serverSettings.loadClientFunc(req.payload.client_id, function (client, err) {

        if (client) {

            // Check client secret

            if ((client.secret || '') === (req.payload.client_secret || '')) {
                ...
                if (req.payload.grant_type === 'client_credentials') {
                    ...
                    if (req.payload.refresh_token) {

                        var refresh = Utils.decrypt(serverSettings.aes256Keys.oauthRefresh, req.payload.refresh_token);
                        if (refresh &&
                            refresh.user &&
                            refresh.client) {
                    ...
                    serverSettings.extensionFunc(req, client, function (user, err, action) {
    ...
    function getOrCreate(user, client, customResponseFields) {
        ...
        if (user === null ||
            ...
            (req.scope && req.scope.authorized === true)) {
...
internals.parseClientCredentials = function (req) {
    ...
    if (authorization) {
        ...
        if (parts.length === 2 &&
            ...
            req.payload.client_id = credentials[0];
            req.payload.client_secret = credentials[1];
            return null;
        }
        else {
        ...
        if (req.payload.client_id) {
            ...
            req.payload.client_secret = req.payload.client_secret || '';
            return null;
        }
        else {
