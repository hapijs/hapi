...
exports.authenticate = function (request, config, next) {
    ...
    MAC.authenticate(request._raw.req, loadTokenFunc, { isHTTPS: serverConfig.tls }, function (isAuthenticated, session, err) {
        ...
        if (!isAuthenticated) {
            ...
            if (isOptional &&
                !request._raw.req.headers.authorization) {

                return next();
            }
            else {

                request._raw.res.setHeader('WWW-Authenticate', MAC.getWWWAuthenticateHeader(err));
...
internals.parseClientCredentials = function (request) {
    ...
    var authorization = request._raw.req.headers.authorization;
    if (authorization) {
