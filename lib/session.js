...
internals.parseClientCredentials = function (request) {
    ...
    if (authorization) {
        if (request.payload.client_id ||
            ...
            return request.reply(Err._oauth('invalid_request', 'Request cannot include both Basic and payload client authentication'));
        }
        ...
        if (parts.length === 2 &&
            ...
            return request.reply(Err._oauth('invalid_request', 'Unsupported HTTP authentication scheme'));
        }
    }
    else {
        if (request.payload.client_id) {
            ...
            return request.reply(Err._oauth('invalid_request', 'Request missing client authentication'));
        }
    }
};
