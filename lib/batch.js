...
module.exports = internals.Batch = function(server) {

    Utils.assert(this.constructor === internals.Batch, 'Batch must be instantiated using new');
    this.server = server;
};

internals.Batch.prototype.batch = function(resultsData, pos, parts, callback) {
    ...
    if (error === null) {
        ...
        self.dispatch('GET', path, null, function (data) {
...
internals.Batch.prototype.dispatch = function (method, path, content, callback) {
    ...
    self.server.inject(injectOptions, callback);
};


internals.Batch.hasRefPart = function(parts) {
...
internals.Batch.prototype.process = function(requests, resultsData, reply) {
    ...
    var callBatch = function(pos, parts) {
        return function(callback) {
            self.batch(resultsData, pos, parts, callback);
        };
    };
    ...
    for(var i = 0, il = requests.length; i < il; ++i) {
        ...
        if(internals.Batch.hasRefPart(parts)) {
...
internals.Batch.prototype.handler = function(request) {
    ...
    if (errorMessage === null) {
        self.process(requests, resultsData, request.reply);
    }
    else {
...
internals.Batch.prototype.endpoint = function() {
