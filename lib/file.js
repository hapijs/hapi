...
exports.load = function (path, options, callback) {
    ...
    Fs.stat(filePath, function (err, stat) {
        ...
        response.header('last-modified', stat.mtime);
        ...
        if (internals.fileEtags.has(cachekey)) {
            response.header('etag', JSON.stringify(internals.fileEtags.get(cachekey)));
        }
        else {
        ...
        if (options.mode) {
            ...
            response.header('content-disposition', options.mode + '; filename=' + encodeURIComponent(fileName));
        }
