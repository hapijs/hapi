...
exports.response = function (path, options, request) {
    ...
    return new Response.Plain(source, request, { variety: 'file', marshall: internals.marshall, prepare: internals.prepare });
};
...
internals.prepare = function (response, callback) {
    ...
    Fs.stat(path, function (err, stat) {
        ...
        if (!response.headers['content-type']) {
            response.type(Mime.path(path).type || 'application/octet-stream');
        }

        response._header('last-modified', stat.mtime.toUTCString());

        if (response._request.server._etags) {
            ...
            var cachedEtag = response._request.server._etags.get(cachekey);
            if (cachedEtag) {
                ...
                response.once('finish', function () {
                    ...
                    response._request.server._etags.set(cachekey, etag);
                });
            }
        }
...
internals.marshall = function (response, callback) {
    ...
    if (!response.source.settings.lookupCompressed ||
        response._request.info.acceptEncoding !== 'gzip') {
