...
Monitor.prototype._loadPackage = function (rootdir) {
    ...
    var filepath = rootdir + '/package.json';
    ...
    if (Fs.existsSync(rootdir)) {
        ...
        try {
            ...
            try {
                ...
                Log.event('err', 'Error parsing package.json at: ' + filepath);
                throw e;
            }
        }
...
Monitor.prototype._initOps = function () {
    ...
    var self = this;
    if (typeof this.settings.ops !== 'undefined' &&
        this.settings.ops !== null) {

        if (this.settings.interval &&
            Object.keys(this.settings.ops).length > 0) {
            ...
            var useDefault = true;
            for (var posturl in Object.keys(this.settings.ops)) {

                if (typeof this.settings.ops[posturl] === 'function') {

                    this.server.on('ops', this.settings.ops[posturl]);
                    useDefault = false;
                }
            }
            ...
            Log.event('err', '\'interval\' setting must be also be supplied with \'ops\'');
        }
    }
    else {
        Log.event('info', 'Hapi will not be monitoring ops data');
    }
}
...
Monitor.prototype._initRequests = function () {
    ...
    if (this.settings.request &&
        Object.keys(this.settings.request).length > 0) {

        var useDefault = true;
        for (var posturl in Object.keys(this.settings.request)) {

            if (typeof this.settings.request[posturl] === 'function') {

                this.server.on('response', this.settings.request[posturl]);
                useDefault = false;
            }
        }
        ...
        Log.event('info', 'Hapi will not be monitoring request responses');
    }
}
...
Monitor.prototype._initLogs = function () {
    ...
    if (this.settings.log &&
        Object.keys(this.settings.log).length > 0) {

        Log.externalStores = this.settings.log;
    }
    else {
        Log.event('info', 'Hapi will be logging to stdout');
    }
}
...
Monitor.prototype._ops = function (client, url) {
    ...
    return function (results) {
        ...
        var module = self.settings.module || 'Blammo';
        var data = {
...
Monitor.prototype.handle = function (event) {
    ...
    return function (request) {
        ...
        if (typeof self.settings[event] !== 'undefined' &&
            self.settings[event] !== null) {

            var hosts = Object.keys(self.settings[event]);
            for (var i in hosts) {
                ...
                if (hosts.hasOwnProperty(i)) {
                    ...
                    if (self.settings[event][host]) {
                        self.settings[event][host](client, self)(request);
                    }
                    else {
