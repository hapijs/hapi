...
module.exports = Monitor = function (server, settings) {
    ...
    if (this.options && this.options.monitor) {
        ...
        Log.info('Hapi monitoring is disabled');
    }
};
    ...
    if (Fs.existsSync(rootdir)) {
        ...
        try {
            ...
            try {
                ...
                Log('Error parsing package.json at: ' + filepath);
                throw e;
            }
        }
...
Monitor.prototype._initOps = function () {
    ...
    if (typeof this.options.monitor.ops !== 'undefined' &&
        ...
        if (this.options.monitor.interval &&
            ...
            Log.err('\'interval\' setting must be also be supplied with \'ops\'');
        }
    }
    else {
        Log.info('Hapi will not be monitoring ops data');
    }
}
...
Monitor.prototype._initRequests = function () {
    ...
    if (this.options.monitor.request &&
        ...
        Log.info('Hapi will not be monitoring request responses');
    }
}
...
Monitor.prototype._initLogs = function () {
    ...
    if (this.options.monitor.log &&
        ...
        Log.info('Hapi will be logging to stdout');
    }
}
