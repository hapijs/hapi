...
internals.Request.prototype._decorate = function (callback) {
    ...
    this.reply = function (result) {
        ...
        Utils.assert(self.response.options.contentLength === undefined, 'Does not support custom content length');
        process(result);
    };
    ...
    this.reply.created = function (uri) {
        ...
        self.response.options.created = (isAbsolute ? url : self.server.settings.uri + (uri.charAt(0) === '/' ? '' : '/') + uri);
        return self.reply;
    };

    this.reply.type = function (type) {
        self.response.options.contentType = type;
        return self.reply;
    };

    this.reply.bytes = function (bytes) {
        self.response.options.contentLength = bytes;
        return self.reply;
    };
};
