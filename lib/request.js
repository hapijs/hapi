...
internals.Request.prototype._setUrl = function (url) {
    ...
    if (this.server.settings.router.normalizeRequestPath === true) {
        this.path = internals.normalizePath(this.url.pathname);
    }
    else {
...
internals.decodeUnreserved = function (encoded) {

    var charDec = parseInt(encoded.substr(1), 16);                                                                          // Remove starting %
    if ((charDec >= 48 && charDec <= 57) || (charDec >= 65 && charDec <= 90) || (charDec >= 97 && charDec <= 122)) {        // [0-9a-zA-Z]
...
internals.normalizePath = function (path) {
    ...
    for (var i = 0, il = path.length; i < il; ++i) {
        if (path[i] === '%' && (i+2 < il)) {
            ...
            if (internals.reservedEncoding[encoded]) {
                ...
                encoded = internals.decodeUnreserved(encoded);
            }

            normalizedPath += encoded;
            i += 2;
        }
        else {
    ...
    var generate = function (callback) {
        ...
        request._prerequisites(function (err) {
            ...
            request._decorate(function (response) {
                ...
                var ttl = (response instanceof Response.Cacheable && response._code === 200 ? response.options.ttl : 0);      // null/undefined: cache defaults, 0: not cached
