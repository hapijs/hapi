...
internals.Request.prototype._execute = function (route) {
    ...
    var funcs = [
        ...
        internals.requisites,                           // Must not call next() with an Error
...
// Pre-fetch requisites 

internals.requisites = function (request, next) {

    var requisites = request._route.config.requisites;
    if (!requisites) {
        return next();
    }
    ...
    */
    ...
    var fetch = function (assign, method, mode) {

        return function (callback) {

            var timer = new Utils.Timer();
            method(request, function (result) {

                if (result instanceof Error) {
                    request.log(['requisites', 'error'], { msec: timer.elapsed(), assign: assign, mode: mode, error: err });
                    return callback(result);
                }

                request.log(['requisites'], { msec: timer.elapsed(), assign: assign, mode: mode });
                if (assign) {
                    request.requisites[assign] = result;
                }
                callback();
            });
        };
    ...
    for (var i = 0, il = requisites.length; i < il; ++i) {

        var requisite = (typeof requisites[i] === 'function' ? { method: requisites[i] } : requisites[i]);

        if (requisite.mode === 'parallel') {                                                        // Defaults to 'serial'
            fnsParallel.push(fetch(requisite.assign, requisite.method, 'parallel'));
        }
        else {
            fnsSerial.push(fetch(requisite.assign, requisite.method, 'serial'));
        }
    }
        ...
        request._decorate(function (response) {
            ...
            var lazySaveToCache = function (item) {
                ...
                if (request._route.cache.isMode('server')) {
                    ...
                    request._route.cache.set(request.url.path, record, item.options.ttl, function (err) {     // 'path' contains query
