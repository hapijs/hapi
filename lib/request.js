...
const internals = {
    properties: ['server', 'url', 'query', 'path', 'method', 'mime', 'setUrl', 'setMethod', 'headers', 'id', 'app', 'plugins', 'route', 'auth', 'pre', 'preResponses', 'info', 'orig', 'params', 'paramsArray', 'payload', 'state', 'jsonp', 'response', 'raw', 'tail', 'addTail', 'domain', 'log', 'getLog', 'generateResponse'],
...
internals.Request.prototype._execute = function () {
    ...
    this._invoke(this.server.root._extensions.route.onRequest, (err) => {
...
internals.Request.prototype._invoke = function (event, callback) {
    ...
    Items.serial(event.nodes, each, callback);
};
...
internals.Request.prototype._finalize = function () {
    ...
    if (Object.keys(this._tails).length === 0) {
        this.server.events.emit('tail', this);
    }
