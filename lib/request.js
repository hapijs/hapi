...
internals.Request = class {
    ...
    constructor(server, req, res, options) {
        ...
        this.setUrl(req.url, this.server.settings.router.stripTrailingSlash);      // Sets: this.url, this.path, this.query
        this.setMethod(req.method);                                                // Sets: this.method

        this.id = this.info.received + ':' + server.info.id + ':' + server._requestCounter.value++;
    ...
    _setTimeouts() {
        ...
        const timeoutReply = () => {
            ...
            this._log(['request', 'server', 'timeout', 'error'], { timeout: serverTimeout, elapsed });
