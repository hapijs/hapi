...
exports = module.exports = internals.Request = class {
    ...
    _listen() {
        ...
        this.raw.req.on('close', internals.abort.bind(this.raw.req, this._eventContext, this.raw.res));
...
internals.abort = function ({ request }, res) {

    // The 'aborted' event on http.IncomingMessage was deprecated in Node.js v17.
    // Use 'close' on req combined with req.destroyed to detect client aborts.
    ...
    if (res.writableEnded) {
        return;         // Response already sent — normal connection close, not an abort
    }

    if (!this.destroyed) {
        return;         // req closed cleanly without being destroyed — not an abort
    }

    internals.event({ request }, 'abort');
};
