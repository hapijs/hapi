...
exports = module.exports = internals.Request = function (server, req, res, options) {
    ...
    if (server.settings.timeout.client) {
        ...
        var clientTimeoutId = setTimeout(function () {

            if (!res._headers) {
                self._error(408, 'Client took too long to send request');
            }
        }, server.settings.timeout.client);
    ...
    if (server.settings.timeout.server) {

        var serverTimeoutId = setTimeout(function () {

            if (!res._headers) {
                self._error(503, 'Server took too long to respond');
            }
        }, server.settings.timeout.server);
...
internals.Request.prototype._error = function (code, message) {

    var self = this;

    // Decorate request with extension helpers

    self._response = self._generateResponse(new Err(code, message));
    self._undecorate();
    self._respond();
    self._wagTail();
};
