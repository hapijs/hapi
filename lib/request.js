    ...
    return function (request, next) {
        ...
        Ext.runProtected(request.log.bind(request), 'pre', next, function (enter, exit) {
            ...
            var finalize = function (result) {
                ...
                return exit(null, result, pre.assign);
            };
...
internals.Request.handler = function (request, next) {
    ...
    var prerequisites = function (callback) {
        ...
        var runPrerequisite = function (pre, nextPre) {

            pre(request, function (err, result, assign) {

                if (err) {
                    return nextPre(err);
                }

                if (result && result.isHapiResponse) {
        ...
        Async.series([
            function (nextSet) {
                ...
                Async.forEach(request._route.prerequisites.parallel, runPrerequisite, nextSet);
            },
            function (nextSet) {

                Async.forEachSeries(request._route.prerequisites.serial, runPrerequisite, nextSet);
            }
        ], function (err) {
    ...
    var generate = function (callback) {
        ...
        prerequisites(function (err, response) {
            ...
            if (response) {
                return callback(null, response, !response.varieties.cacheable);
            }
