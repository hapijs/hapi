...
internals.Request.prototype._setCache = function(headers) {
    ...
    var cacheRules = (this._route && this._route.cache) ? this._route.cache.rules : null;

    if (cacheRules !== null && Rules.isCached(this.url.path, cacheRules, 'headers')) {
        var expiresInSec = Rules.expireInSec(this.url.path, cacheRules);
