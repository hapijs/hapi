...
internals.Request.prototype._execute = function () {
    ...
    Handler.invoke(this, 'onRequest', function (err) {
        ...
        if (self.raw.req.socket &&
            self.route.timeout.socket !== undefined) {

            self.raw.req.socket.setTimeout(self.route.timeout.socket || 0);     // Value can be false or positive
        }

        var serverTimeout = self.route.timeout.server;
        if (serverTimeout) {
