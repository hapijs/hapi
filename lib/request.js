...
internals.Request.prototype._setCors = function (rawRes) {
    ...
    if (this.server.settings.cors &&
        ...
        rawRes.setHeader('Access-Control-Allow-Origin', this.server.settings.cors._origin);
        rawRes.setHeader('Access-Control-Max-Age', this.server.settings.cors.maxAge);
        rawRes.setHeader('Access-Control-Allow-Methods', this.server.settings.cors._methods);
        rawRes.setHeader('Access-Control-Allow-Headers', this.server.settings.cors._headers);
    }
};
...
internals.Request.prototype._setCache = function(rawRes) {
    ...
    if (this._route &&
        ...
        rawRes.setHeader('Cache-Control', 'max-age=' + Math.floor(this.response.options.ttl / 1000));       // Convert MSec ttl to Sec in HTTP header
    }
    else {
        rawRes.setHeader('Cache-Control', 'must-revalidate');
    }
};
...
internals.Request.prototype._respond = function () {
    ...
    var rawRes = self.raw.res;
    var code = 200;
    var data = null;

    var review = function () {

        self._setCors(rawRes);
        self._setCache(rawRes);
        ...
        if (result) {
            ...
            if (result instanceof Error) {
                ...
                if (self.response.options.created) {
                    code = 201;
                    rawRes.setHeader('Location', self.response.options.created);
                }

                if (self.response.options.contentType) {
                    rawRes.setHeader('Content-Type', self.response.options.contentType);
                }

                if (self.response.options.contentLength) {
                    rawRes.setHeader('Content-Length', self.response.options.contentLength);
                }
    ...
    var prepare = function (contentType) {
        ...
        if (data !== null &&
            !rawRes.getHeader('Content-Length')) {                                       // data can be empty string

            rawRes.setHeader('Content-Length', Buffer.byteLength(data));
        }
    ...
    var send = function () {
        rawRes.writeHead(code);
        rawRes.end(self.method !== 'head' ? data : '');
