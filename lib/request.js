...
internals.Request.prototype._reply = function (exit) {
    ...
    var process = function () {
        ...
        if (exit) {
            self._setResponse(Response._generate(exit, self));
        }
        ...
        self.server._ext.invoke(self, 'onPreResponse', function (err) {
            ...
            if (err) {                                         // err can be valid response or error
                self._setResponse(Response._generate(err, self));
            }
    ...
    var finalize = function () {
        ...
        if (self._response &&
            ((self._response.isBoom && self._response.response.code === 500) ||
             (self._response.variety === 'error' && self._response._code === 500))) {
...
internals.Request.prototype.generateResponse = function (result) {
    ...
    return Response._generate(result, this);
};
