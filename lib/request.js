...
internals.Request.prototype.log = function (tags, data, timestamp) {
    ...
    this._log.push(item);
    this.server.emit('request', this, item);
};
...
internals.Request.prototype._execute = function (route) {
    ...
    this.raw.req.once('error', function (err) {

        self._reply(Boom.internal('Request error before finished: ' + err));
    });

    this.raw.res.once('error', function (err) {

        self._reply(Boom.internal('Response error before finished: ' + err));
    });
