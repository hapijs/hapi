...
exports = module.exports = internals.Request = function (server, req, res, options) {
    ...
    this._log(['http', 'received'], about, now);     // Must be last for object to be fully constructed
    ...
    if (err) {
        if (self.response &&
            ...
            self._route.cache.drop(self.url.path, function (err) {
                ...
                self._log(['request', 'cache', 'drop'], err);
            });
        }
...
internals.handler = function (request, next) {
    ...
    var log = request._log.bind(request);
            ...
            Ext.runProtected(log, 'handler', callback, function (run, next) {
                ...
                var finalize = function (response) {
                    ...
                    if (response &&
                        ...
                        request._log(['handler', 'result', 'error'], { msec: timer.elapsed() });
                    ...
                    request._log(['handler', 'result'], { msec: timer.elapsed() });
...
internals.Request.prototype._addTail = function (name) {
    ...
    var drop = function () {
        ...
        if (!self._tails[tailId]) {
            // Already removed
            self._log(['tail', 'remove', 'error'], { name: name, id: tailId });
            return;
        }
