...
exports = module.exports = internals.Request = function (server, req, res, options) {
    ...
    this._setUrl(req.url, server.settings.router.stripTrailingSlash);       // Sets: this.url, this.path, this.query, this._pathSegments
    ...
    this._route = this.server._notfound;    // Used prior to routing (only settings are used, not the handler)
...
internals.Request.prototype._execute = function () {
    ...
    Handler.invoke(this, 'onRequest', function (err) {
        ...
        if (match.isBoom) {
            if (match.output.statusCode !== 404) {
                return self._reply(match);
            }
        }
        else {
