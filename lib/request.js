...
internals.Request = function (server, req, res, options) {
...
internals.Request.prototype._execute = async function () {
    ...
    for (let i = 0; i < this._route._cycle.length; ++i) {
        ...
        try {
            ...
            else if (typeof func !== 'function') {                              // Extension point
                response = await this._invoke(func);
            }
            else {
                response = await new Promise((resolve, reject) => func(this, Promises.split(resolve, reject)));
            }
        }
        catch (err) {
            response = err;
...
internals.Request.prototype._invoke = async function (event) {
    ...
    for (let i = 0; i < event.nodes.length; ++i) {
        ...
        const reply = this.server._blah.interface(this, ext.plugin.realm);
        ...
        let response;
        try {
            response = await ext.func.call(bind, this, reply);
        }
        catch (err) {
        ...
        if (response === undefined) {
            response = Boom.badImplementation(`${event.type} extension method did not return a value or a promise`);
        }

        if (typeof response === 'symbol') {
            if (response === this.server._blah.continue) {
                continue;
            }
        ...
        response = Response.wrap(response, this);
        if (!response.isBoom) {
...
internals.Request.prototype._reply = async function (exit) {
    ...
    if (this.response === this.server._blah.close) {
            this.raw.res.end();                                     // End the response in case it wasn't already closed
    }

    if (this.response === this.server._blah.abandon ||
        this.response === this.server._blah.close) {
