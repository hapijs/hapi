...
internals.Request.prototype.getLog = function (tags, internal) {
    ...
    for (let i = 0, il = this._logger.length; i < il; ++i) {
        const event = this._logger[i];
        if (internal === undefined || event.internal === internal) {
            if (filter) {
                for (let t = 0, tl = event.tags.length; t < tl; ++t) {
...
internals.Request.prototype._invoke = function (event, callback) {
    ...
    this._protect.run(callback, function (exit) {
        ...
        Items.serial(event.nodes, function (ext, next) {
            ...
            const reply = self.server._replier.interface(self, ext.plugin.realm, next);
