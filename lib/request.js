...
internals.Request.prototype._execute = function (route) {
    ...
    function (err) {
        ...
        if (err) {
            if (self.response &&
                !(self.response instanceof Error)) {
...
internals.Request.prototype._decorate = function (callback) {
    ...
    this.reply.stream = function (stream) {
        ...
        response = Response.generateResponse(stream, process);
        return response;
    };
    ...
    this.reply.payload = function (result) {
        ...
        response = Response.generateResponse(result, process);
        return response;
    };
};
...
internals.Request.prototype._unknown = function () {
    ...
    if (!this.server.settings.ext.onUnknownRoute) {
        ...
        this.response = Err.notFound('No such path or method ' + this.path);
        this._respond();
        this._wagTail();
        return;
    }
    ...
    var lookup = function () {
        ...
        request._route.cache.getOrGenerate(request.url.path, logFunc, generate, function (response, cached) {  // request.url.path contains query
            ...
            if (!(response instanceof Error)) {
                if (cached) {
                    response.ttl(cached.ttl);
                }
                else if (request._route.cache.isMode('client')) {
                    response.ttl(request._route.cache.ttl(), false);        // Does not override ttl is already set
                }
            }
    ...
    var generate = function (callback) {
        ...
        request._prerequisites(function (err) {
            ...
            request._decorate(function (response) {
                ...
                return callback(null, response, response._ttl);                     // Not all response types include _ttl
            });
...
internals.Request.prototype._respond = function () {
    ...
    Response._respond(this.response, this, function () {
