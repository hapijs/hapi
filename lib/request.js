...
internals.Request = class {
    ...
    _listenRequest() {
        ...
        this.raw.req._hapiRequest = this;

        this.raw.req.on('end', internals.onEnd);
        this.raw.req.on('close', internals.onClose);
        this.raw.req.on('error', internals.onError);
        this.raw.req.on('aborted', internals.onAbort);
    }
    ...
    _finalize() {
        ...
        this.raw.req._hapiRequest = null;
...
internals.info = function (server, req) {
    ...
    info.id = info.received + ':' + server.info.id + ':' + server._requestCounter.value++;
...
internals.onEnd = function () {

    if (this._hapiRequest) {
        this._hapiRequest._isPayloadPending = false;
    }
};
...
internals.onClose = function () {
    ...
    if (this._hapiRequest) {
        this._hapiRequest._log(['request', 'closed', 'error']);
        this._hapiRequest._isPayloadPending = false;
        this._hapiRequest._isBailed = true;
    }
};
...
internals.onError = function (err) {
    ...
    if (this._hapiRequest) {
        this._hapiRequest._log(['request', 'error'], err);
        this._hapiRequest._isPayloadPending = false;
    }
};


internals.onAbort = function () {

    if (this._hapiRequest) {
        this._hapiRequest._log(['request', 'abort', 'error']);
        this._hapiRequest._isPayloadPending = false;
        this._hapiRequest._isBailed = true;

        if (this._hapiRequest._events) {
            this._hapiRequest._events.emit('disconnect');
        }
    }
};
