...
internals.Request.prototype._decorate = function (callback) {
    ...
    this.reply.view = function (templateName, context) {

        // response = self._generateResponse(self.server.Views.render(templateName, context));
        response = self._generateResponse(new Response.View(templateName, context), process);
        return response;
    };
};
