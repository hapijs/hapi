...
internals.Request.handler = function (request, next) {
    ...
    var lookup = function () {
        ...
        var generateKey = function (request) {
            var key = request.url.pathname;
            var vary = request._route.settings.cache.varyByParam;
            ...
            for(var i = 0; i < vary.length; i++){
                if(headers[vary[i]]){
                    key += (vary[i] + '=' + encodeURIComponent(headers[vary[i]]) + '&');
                }
            }
        ...
        request._route.cache.getOrGenerate(generateKey(request), generate, function (err, value, cached, report) {  // request.url.path contains query
...
internals.Request.prototype._decorateReply = function (finalize) {
    ...
    this.reply = function (result) {
        ...
        return Response._generate(result, self, finalize);
    };
    ...
    this.reply.redirect = function (uri) {
        ...
        return Response._generate(new Response.Redirection(uri), self, finalize);
    };
