...
internals.Request.prototype._setCache = function(headers) {
    ...
    if (this._route && this._route.cache.isMode('client')) {
        var expiresInSec = Rules.expireInSec(this._route.cache.rule);

        if (this.response && this.response.resultCreated) {
            var cachedTime = Date.now() - new Date(this.response.resultCreated);
            expiresInSec = cachedTime ? expiresInSec - new Date(cachedTime).getSeconds() : expiresInSec;
        }
