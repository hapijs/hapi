...
internals.Request.prototype._decorate = function (callback) {
    ...
    this.created = function (uri) {
        ...
        self.response.created = (isAbsolute ? url : self.server.settings.uri + (uri.charAt(0) === '/' ? '' : '/') + uri);
    };

    self.contentType = function (type) {

        self.response.created = type;
    };

    self.contentLength = function (length) {
