...
exports = module.exports = internals.Request = class {
    ...
    constructor(server, req, res, options) {
        ...
        this._hostError = null;                                                                             // Assigned when host and :authority disagree
    ...
    async _onRequest() {
        ...
        // Validate host before anything gets a chance to route on it
...
internals.Info = class {
    ...
    constructor(request) {
        ...
        const authority = (req.headers[':authority'] || '').trim();
        const host = (req.headers.host || '').trim() || authority;
        const received = Date.now();

        // RFC 9113 8.3.1 - a request carrying both host and :authority is malformed when they disagree,
        // otherwise a front-end routing on one and hapi routing on the other can be desynced

        if (!internals.isSameAuthority(host, authority, req.headers[':scheme'] ?? request._core.info.protocol)) {
            request._hostError = Boom.badRequest('Host header does not match :authority pseudo-header');
        }
...
internals.isSameAuthority = function (host, authority, scheme) {
    ...
    // RFC 3986 6.2.3 - the scheme's default port is equivalent to leaving the port out

    const port = scheme === 'https' ? ':443' : ':80';
    const normalize = (value) => {

        value = value.toLowerCase();
        return value.endsWith(port) ? value.slice(0, -port.length) : value;
    };

    return normalize(host) === normalize(authority);
};
