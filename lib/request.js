...
internals.Request.prototype._decorate = function (callback) {
    ...
    var process = function (result, options) {
        ...
        if (options) {
            self.response.options = self.response.options || {};
            ...
            if (options.created) {
                ...
                self.response.options.created = (isAbsolute ? url : self.server.settings.uri + (options.created.charAt(0) === '/' ? '' : '/') + options.created);
            }

            if (options.contentType) {
                self.response.options.contentType = options.contentType;
            }

            if (options.contentLength !== undefined) {      // Can be zero
                self.response.options.contentLength = options.contentLength;
            }
        }
    ...
    this.reply = function (result, options) {
        ...
        Utils.assert(!(result instanceof Stream), 'request.reply() cannot take a stream');
        Utils.assert(!options || !options.contentLength, 'request.reply() does not support custom content length');
...
internals.handler = function (request, next) {
    ...
    request._route.cache.get(request.url.path, function (err, cached) {           // path contains query
        ...
        if (cached &&
            ...
            request.log(['cache', 'get'], { found: true, msec: timer.elapsed(), created: cached.created, ttl: cached.ttl });
            return next();
        }
        else {
...
internals.Request.prototype._setCache = function(headers) {
    ...
    if (this._route &&
        ...
        headers['Cache-Control'] = 'max-age=' + this.response.ttl;
    }
    else {
