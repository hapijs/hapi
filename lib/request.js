...
internals.Request.prototype._execute = function (route) {
    ...
    var funcs = [
        ...
        internals.prerequisites,                        // Must not call next() with an Error
...
internals.prerequisites = function (request, next) {

    var prerequisites = request._route.config.pre;
    if (!prerequisites) {
        return next();
    }
    ...
    */
        ...
        return function (callback) {
            ...
            method(request, function (result) {
                ...
                if (result instanceof Error) {
                    request.log(['prerequisites', 'error'], { msec: timer.elapsed(), assign: assign, mode: mode, error: err });
                    return callback(result);
                }

                request.log(['prerequisites'], { msec: timer.elapsed(), assign: assign, mode: mode });
                if (assign) {
                    request.pre[assign] = result;
                }
                callback();
            });
        };
    ...
    for (var i = 0, il = prerequisites.length; i < il; ++i) {

        var pre = (typeof prerequisites[i] === 'function' ? { method: prerequisites[i] } : prerequisites[i]);

        if (pre.mode === 'parallel') {                                                        // Defaults to 'serial'
            fnsParallel.push(fetch(pre.assign, pre.method, 'parallel'));
        }
        else {
            fnsSerial.push(fetch(pre.assign, pre.method, 'serial'));
        }
    }
