...
exports = module.exports = internals.Request = function (server, req, res, options) {
    ...
    // rawBody
    // payload
    // session: { app, scope, user, ext.tos }
...
internals.Request.prototype._respond = function () {
    ...
    var review = function () {
        ...
        if (result instanceof Error) {
            ...
            if (self.server.settings.errors &&
                ...
                self.server.settings.errors.format(result, function (errCode, errPayload, errContentType) {
                ...
                var errResponse = (result instanceof Err ? result.toResponse() : Err.toResponse(result));
                code = errResponse.code || 500;
    ...
    var send = function () {
        ...
        self.raw.res.end(self.method !== 'head' ? payload : '');
    ...
    var stream = function () {
        ...
        // Check if payload is a node HTTP response (payload.*) or a (mikeal's) Request object (payload.response.*)
        ...
        if (!self._route ||
            ...
            var responseHeaders = payload.response ? payload.response.headers : payload.headers;
            if (responseHeaders) {
                Utils.merge(headers, responseHeaders);
            }
        }

        code = payload.statusCode || ((payload.response && payload.response.code) ? payload.response.code : code);

        self.raw.res.writeHead(code, headers);

        self.raw.req.on('close', function () {

            payload.destroy.bind(payload);
        });
