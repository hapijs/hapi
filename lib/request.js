...
exports = module.exports = internals.Request = function (server, req, res, options) {
    ...
    if (server.settings.timeout.client) {

        var clientTimeoutId = setTimeout(this._clientTimeoutError.bind(this), server.settings.timeout.client);
...
internals.startServerTimeout = function (request, next) {

    var timeout = request.server.settings.timeout.server;
    if (timeout) {
        request._serverTimeoutId = setTimeout(request._serverTimeoutError.bind(request), timeout);
    }
...
internals.Request.prototype._clientTimeoutError = function () {

    this._response = this._generateResponse(new Err(408, 'Client is taking too long to send request'));
    this._undecorate();
    this._respond();
    this._wagTail();
};
...
internals.Request.prototype._serverTimeoutError = function () {

    this._response = this._generateResponse(new Err(503, 'Server is taking too long to respond'));
    this._undecorate();
    this._respond();
    this._wagTail();
};
