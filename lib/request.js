...
internals.Request.prototype._ext = function (event, callback) {
    ...
    Async.forEachSeries(handlers, function (func, next) {
        ...
        self._runProtected(event, next, function (run, protectedNext) {

            run(function () {
                
                func(self, protectedNext);
            });
        });
    },
    function (err) {
...
internals.Request.prototype._runProtected = function (tags, callback, setup) {
    ...
    setup(function (run) {
        ...
        domain.on('error', function (err) {
            ...
            domain.dispose();
            self.log(['uncaught'].concat(tags || []), err);
