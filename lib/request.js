...
exports = module.exports = internals.Request = function (server, req, res, options) {
    ...
    this._route = self.server._router.notfound;             // Used prior to routing (only settings are used, not the handler)
    this.route = self._route.settings;
...
internals.Request.prototype._reply = function (exit) {
    ...
    var finalize = function () {
        ...
        self.raw.req.destroy();                 // In case there was any unread data pending
    };
...
internals.Request.jsonp = function (request, next) {
        ...
        prerequisites(function (err) {
            ...
            Ext.runProtected(request.log.bind(request), 'handler', callback, function (enter, exit) {
                ...
                request.reply = function (result) {
                    ...
                    response = Response._generate(result, finalize);
                    return response;
                };
