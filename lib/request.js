...
internals.createTimeout = function (request, next) {

    if (request.server.settings.timeout) {
        request._timeoutId = setTimeout(function () {

            request.reply(new Err(503, 'Server took too long to respond'));
        }, request.server.settings.timeout);
    }
