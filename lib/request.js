...
internals.prerequisites = function (request, next) {
    ...
    if (request._route.prerequisites.length === 0) {
        return next();
    }
    ...
    */
    ...
    var fetch = function (pre) {
        ...
        return function (callback) {
            ...
            pre.method(request, function (result) {

                if (result instanceof Error) {
                    request.log(['prerequisites', 'error'], { msec: timer.elapsed(), assign: pre.assign, mode: pre.mode, error: err });
    ...
    for (var i = 0, il = request._route.prerequisites.length; i < il; ++i) {

        var pre = request._route.prerequisites[i];
