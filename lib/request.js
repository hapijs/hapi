...
internals.handler = function (request, next) {
    ...
    var call = function () {
        ...
        request._decorate(function (response) {
            ...
            if (request._isHandledStale) {                                                     // Already sent stale response
    ...
    request._route.cache.get(request.url.path, function (err, cached) {           // path contains query
        ...
        var sendCached = function () {
            ...
            request.log(['cache', 'get'], { msec: timer.elapsed(), created: cached.created, ttl: cached.ttl, isStale: cached.isStale });
        ...
        cached.ttl -= Math.ceil(request._route.cache.rule.staleTimeoutMSec / 1000);                 // Adjust TTL for when the timeout is invoked
        ...
        setTimeout(handlerTimeoutFunc, request._route.cache.rule.staleTimeoutMSec);                 // Trigger timeout
