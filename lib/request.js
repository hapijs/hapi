...
internals.Request.prototype._execute = function (route) {
    ...
    var execute = function () {
        ...
        var funcs = [
            ...
            Validation.payload,
            Validation.path,
            ext(self.server.settings.ext.onPreHandler),
            internals.handler,                                      // Must not call next() with an Error
            ext(self.server.settings.ext.onPostHandler),            // An error from here on will override any result set in handler()
            Validation.response
        ];
...
internals.authValidatePayload = function (request, next) {
    ...
    else if (config.payload === 'optional' && (!request.session || !request.session.attributes || request.session.attributes.hash)) {
        return next();
    }
