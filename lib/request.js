...
internals.Request.prototype.log = function (tags, data, timestamp, _internal) {
    ...
    this.connection.emit({ name: _internal ? 'request-internal' : 'request', tags: tagsMap }, [this, event]);
...
internals.Request.prototype._invoke = function (event, callback) {
    ...
    this._protect.run(callback, (exit) => {
        ...
        const each = (ext, next) => {
            ...
            const reply = this.server._replier.interface(this, ext.plugin.realm, {}, next);     // next() called with response object which ends processing (treated like error)
