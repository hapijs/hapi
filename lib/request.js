...
internals.Request.prototype._execute = function (route) {
    ...
    this._route = route;
    var isProcessed = false;                            // Only process async results once

    if (self.server.settings.timeout.server) {
        self._serverTimeoutId = setTimeout(function () {

            processAsyncResults(Err.serverTimeout('Server is taking too long to respond'));
    ...
    var processAsyncResults = function (err) {

        if (isProcessed) {                              // Prevent any future responses to this request
            return;
        }
        else {
            isProcessed = true;
    ...
    Async.forEachSeries(funcs, funcsIterator, processAsyncResults);
};
