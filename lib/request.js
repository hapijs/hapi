...
exports = module.exports = internals.Request = function (connection, req, res, options) {
    ...
    if (req.socket &&
        connection.settings.timeout.socket !== undefined) {

        req.socket.setTimeout(connection.settings.timeout.socket || 0);     // Value can be false or positive
    }
...
internals.Request.prototype.log = function (tags, data, timestamp) {
    ...
    this.connection.emit('request', this, event, tagsMap);

    if (this.server._core._settings.debug &&
        this.server._core._settings.debug.request &&
        Hoek.intersect(tagsMap, this.server._core._settings.debug.request, true)) {
...
internals.Request.prototype._execute = function () {
    ...
    Handler.invoke(this, 'onRequest', function (err) {
        ...
        var serverTimeout = self.connection.settings.timeout.server;
        if (serverTimeout) {
...
internals.Request.prototype._setResponse = function (response) {
    ...
    if (response.isBoom &&
        ...
        this.connection.emit('internalError', this, response);
