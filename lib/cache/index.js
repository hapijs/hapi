        ...
        if (config.expiresAt) {
            if (config.expiresInSec === undefined) {
                ...
                if (time &&
                    ...
                    if (config.staleInSec && config.staleInSec > 86400) {                                   // staleInSec should be less than a day
                        return new Error('staleInSec must be less than 86400 seconds');
                    }
                    else {
            ...
            if (config.staleInSec && config.staleInSec > config.expiresInSec) {
                return new Error('staleInSec must be less than expiresInSec');
            }
            else {
                rule.staleInSec = config.staleInSec;
            }
