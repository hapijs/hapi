...
exports.compile = function (config) {
    ...
    */
    ...
    Utils.assert(!config.staleTimeoutMSec || !config.expiresInSec || Math.ceil(config.staleTimeoutMSec / 1000) < config.expiresInSec, 'staleTimeoutMSec must be less than expiresInSec');
    Utils.assert(!config.staleTimeoutMSec || !config.expiresInSec || Math.ceil(config.staleTimeoutMSec / 1000) < config.expiresInSec - config.staleInSec, 'staleTimeoutMSec must be less than the delta between expiresInSec and staleInSec');
...
exports.ttl = function (rule, created) {
    ...
    var now = Date.now();
    created = created || now;
    var age = Math.ceil((now - created) / 1000);
    ...
    if (rule.expiresAt) {
        ...
        var expiresInSec = Math.floor((expiresAt.getTime() - created) / 1000);
        if (expiresInSec <= 0) {
