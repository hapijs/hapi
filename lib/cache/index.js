...
Client.prototype.get = function (key, rule, callback) {
    ...
    if (this.connection) {
        this.connection.get(key, function (err, item, created) {
            ...
            if (err) {
                // Connection error
                return callback(err, { item: null, isStale: false });
            }
            ...
            if (item) {
                if (Rules.getTtl(rule, created) > 0) {
                    ...
                    return callback(null, { item: item, created: created, isStale: false });     // err, itemData
                }
                else {
                    // Expired
                    return callback(null, { item: null, isStale: false });
                }
            }
            else {
                // Not found
        ...
        // Disconnected
        return callback('Disconnected', { item: null, isStale: false });
    }
};
