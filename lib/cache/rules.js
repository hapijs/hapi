...
exports.getTtl = function (rule, created) {
    ...
    if (age >= 0) {
        if (rule.expiresInSec) {
            ...
            var expiresAt = new Date(now);                                              // Assume everything expires in relation to now
            expiresAt.setHours(rule.expiresAt.hours);
            expiresAt.setMinutes(rule.expiresAt.minutes);
            expiresAt.setSeconds(0);

            if ((expiresAt - now) <= 0 && created <= expiresAt) {                       // If expiresAt already happened and created is before the expiresAt
                return 0;
            }
