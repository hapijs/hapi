...
internals.cors = function (response) {
    ...
    response._header('access-control-allow-methods', cors._methods, config);
    response._header('access-control-allow-headers', cors._headers, config);

    if (cors._exposedHeaders.length !== 0) {
        response._header('access-control-expose-headers', cors._exposedHeaders, config);
    }
};
