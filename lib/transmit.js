...
internals.marshal = function (request, next) {
    ...
    var mime = Mimos.type(response.headers['content-type'] || 'application/octet-stream');
    var encoding = (mime.compressible && !response.headers['content-encoding'] ? request.info.acceptEncoding : null);
    response._contentEncoding = (encoding === 'identity' ? null : encoding);

    if (response._contentEncoding
        && response._isPayloadSupported()) {

        response._header('content-encoding', response._contentEncoding);
        response.vary('accept-encoding');
    }

    response._varyEtag();

    // Response 'etag' and 'last-modified' cannot change past this point
...
internals.transmit = function (response, callback) {
    ...
    if (response._contentEncoding
        ...
        delete response.headers['content-length'];
        compressor = (response._contentEncoding === 'gzip' ? Zlib.createGzip() : Zlib.createDeflate());
    }
