...
internals.cache = function (response) {
    ...
    var request = response.request;
    if ((request._route._cache && request.route.cache._statuses[response.statusCode]) ||
        response.settings.ttl) {

        var ttl = (response.settings.ttl !== null ? response.settings.ttl : request._route._cache.ttl());
        var privacy = (request.auth.isAuthenticated || response.headers['set-cookie'] ? 'private' : request.route.cache.privacy || 'default');
...
internals.security = function (response) {
    ...
    var security = request.route.security;
    if (security) {
        if (security._hsts) {
...
internals.state = function (response, next) {
    ...
    function (err) {
        ...
        request.connection._stateDefinitions.format(states, function (err, header) {
