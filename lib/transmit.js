...
internals.pipe = function (request, stream) {
    ...
    request.raw.req._hapiEnding = env;
    request.raw.res._hapiEnding = env;
    stream._hapiEnding = env;

    request.raw.req.on('aborted', internals.onAborted);
    request.raw.req.on('close', internals.onClose);
    request.raw.res.on('close', internals.onClose);
    request.raw.res.on('error', internals.end);
    request.raw.res.on('finish', internals.end);
...
internals.onAborted = function () {

    internals.end.call(this, null, 'aborted');
};


internals.onClose = function () {

    internals.end.call(this, null, 'close');
};


internals.end = function (err, event) {

    if (!this._hapiEnding) {                                                            // Used instead of cleaning up emitter listeners
        return;
    }

    const { stream, request, team } = this._hapiEnding;

    request.raw.req._hapiEnding = null;
    request.raw.res._hapiEnding = null;
    stream._hapiEnding = null;
...
internals.chain = function (sources) {
    ...
    for (let i = 1; i < sources.length; ++i) {
        ...
        if (to) {
            from._hapiPipe = to;
            from.on('error', internals.errorPipe);
            from = from.pipe(to);
        }
    }
...
internals.errorPipe = function (err) {

    this._hapiPipe.emit('error', err);
    this._hapiPipe = null;
};
