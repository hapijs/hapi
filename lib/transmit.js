...
internals.transmit = function (response) {
    ...
    if (isInjection) {
        request.raw.res[Plugin._symbol] = { request };

        if (response.variety === 'plain') {
            request.raw.res[Plugin._symbol].result = response._isPayloadSupported() ? response.source : null;
        }
    }
...
internals.encoding = function (response, encoding) {
    ...
    return request.server.root._compression.encoder(request, encoding);
};
...
internals.end = function (env, event, err) {
    ...
    if (err) {
        ...
        if (request.raw.res[Plugin._symbol]) {
            request.raw.res.statusCode = 500;
            request.raw.res[Plugin._symbol].result = Boom.boomify(err).output.payload;      // Force injected response to error
        }
