...
exports = module.exports = internals.Auth = class {
    ...
    async test(name, request) {
        ...
        const response = await request._core.responder.execute(strategy.methods.authenticate, request, { bind, realm, auth: true });
    ...
    static async payload(request) {
        ...
        const bind = strategy.methods;
        const realm = strategy.realm;
        const response = await request._core.responder.execute(strategy.methods.payload, request, { bind, realm });
    ...
    static async response(request) {
        ...
        const bind = strategy.methods;
        const realm = strategy.realm;
        const error = await request._core.responder.execute(strategy.methods.response, request, { bind, realm, continue: 'undefined' });
        if (error) {
            throw error;
        }
    }
