...
internals.Auth.prototype.strategy = function (name, scheme, ...args /*, mode, options */) {
...
internals.Auth.prototype.test = async function (name, request) {
...
internals.Auth.prototype._authenticate = function (request) {
...
internals.Auth.response = async function (request) {
    ...
    const bind = strategy.methods;
    const realm = strategy.realm;
    return await request.server._responder.execute(strategy.methods.response, request, { bind, realm, continue: 'undefined' });
};
...
internals.validate = function (err, result, name, config, request, errors) {                 // err can be Boom, Error, or a valid response object
    ...
    if (!err &&
        ...
        throw Boom.badImplementation('Authentication response missing both error and credentials');
    }
