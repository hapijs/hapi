...
internals.Auth.prototype.test = function (name, request, next) {
    ...
    const transfer = function (response, data) {
...
internals.Auth.prototype._setupRoute = function (options, path) {
    ...
    if (options.scope) {
        ...
        for (let i = 0, il = options.scope.length; i < il; ++i) {
...
internals.Auth.prototype._authenticate = function (request, next) {
    ...
    const validate = function (err, name, result) {           // err can be Boom, Error, or a valid response object
        ...
        if (config.scope) {
            ...
            if (config.hasScopeParameters) {
                ...
                for (let i = 0, il = config.scope.length; i < il; ++i) {
