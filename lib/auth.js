...
internals.Auth.prototype.test = function (name, request, next) {
    ...
    const transfer = (response, data) => next(response, data && data.credentials);
    const reply = request.server._replier.interface(request, strategy.realm, { data: true }, transfer);
    ...
    execute(next) {
        ...
        if (this.current < config.strategies.length) {
            ...
            request._protect.run(after, (exit) => {
                ...
                const reply = request.server._replier.interface(request, strategy.realm, { data: true }, exit);
                strategy.methods.authenticate(request, reply);
            });
