...
internals.setupScope = function (access) {
    ...
    for (let i = 0; i < access.scope.length; ++i) {
        ...
        if (typeof value === 'string') {
            ...
            // I pass an array with the string type as the first element
            // and a function as the next
            const clean = value[1];
            scope[type].push(clean);
            scope._parameters = scope._parameters || {};
            scope._parameters[type] = true;
...
internals.expandScopeType = function (request, scope, type) {
    ...
    for (let i = 0; i < scope[type].length; ++i) {
        const scopeItem = scope[type][i];
        if (typeof scopeItem === 'string') {
            expanded.push(Hoek.reachTemplate(context, scopeItem));
        }
        if (typeof scopeItem === 'function') {
            expanded.push(scopeItem(context));
        }
    }
