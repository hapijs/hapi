...
internals.Auth.prototype.test = async function (name, request, next) {
    ...
    const response = await request.server._blah.execute(strategy.methods.authenticate, request, { bind, realm, auth: true });

    if (!response.isAuth) {
        return next(response);
    }

    return next(response.error, response.data && response.data.credentials);
};
...
internals.Auth.payload = async function (request) {
    ...
    const bind = strategy.methods;
    const realm = strategy.realm;
    const response = await request.server._blah.execute(strategy.methods.payload, request, { bind, realm });

    if (response === request.server._blah.continue) {
        return;
    }
...
internals.Auth.response = async function (request) {
    ...
    const bind = strategy.methods;
    const realm = strategy.realm;
    const response = await request.server._blah.execute(strategy.methods.response, request, { bind, realm });
...
internals.Authenticator = class {
    ...
    authenticate(next) {
        ...
        return this.execute(next).then(Hoek.ignore);
    }
    ...
    async execute(next) {
        ...
        if (this.current < config.strategies.length) {
            ...
            const response = await request.server._blah.execute(strategy.methods.authenticate, request, { bind, realm, auth: true });
            if (!response.isAuth) {
                return this.validate(response, null, next);
            }

            return this.validate(response.error, response.data, next);
        }
