    ...
    if (this.isInitialized !== true) {
        ...
        process.on('uncaughtException', function (err) {
            ...
            if (self.settings.email &&
                self.settings.email.admin) {

                Email.send(self.settings.email.admin, 'ERROR: Exception on ' + self.settings.name + ' server', err.stack, '', self.settings.email, function (mailErr) {
...
Process.prototype.finalize = function (onCompleted) {
    ...
    if (this.settings.email &&
        ...
        Email.send(this.settings.email.admin, 'NOTICE: ' + this.settings.name + ' server started', 'Started on ' + Os.hostname(), '', this.settings.email);
    }
