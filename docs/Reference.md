...
- `host` - the hostname, IP address, or path to unix domain socket the server is bound to. Defaults to `0.0.0.0` which means any available network
  ...
  path to the socket). If the argument contains a `\\.\pipe`, then it is assumed to be a windows named pipe and it will bind to this much like 
  unix domain sockets. (`server.info.namedPipe` will contain the path to the named pipe)
