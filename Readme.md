...
- `mode` - determines if the matching resource is cached on the server, client, or both. Defaults to server+headers. Can be used to exclude a subset of resources from caching.
    * `server+headers` - Caches the route response on the server and client (default)
    * `headers` - Sends the Cache-Control HTTP header on the response to support client caching
...
If a route needs to only be cached on the client set the `mode` to _`headers`_ .  If a route should be cached on both the client and server set the `mode` property to _`server+headers`_ .  To enable caching a route only on the server set the `mode` to _`server`_ .  To disable cache altogether for a route set `mode` to _`none`_ .
