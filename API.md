...
The `'closing'` event type is emitted when the server is stopped using [`server.stop()`](#server.stop()) but before all underlying connections have been closed thus before the [`'stop'`](#server.events.stop) event is triggered.
